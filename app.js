...
function maybeEnhanceWithApi(rawText, token) {
  ...
  callStoredKeyApi([
    {
      role: "system",
      content: "你是日精進復盤教練。只輸出 JSON，繁體中文。需含 themeCategory、themeTitle、themeInsight、problems、eventList、reactionList、reflection、conclusion、quotes、gratitudeNote、sfm、tags。",
    },
  ...
  root.innerHTML = `
    <section class="review-section" aria-labelledby="sec-theme">
      ...
      <article class="theme-banner">
        <p class="theme-banner__kicker">精準拆解盲點</p>
...
function buildCoachQuotes({ category, hasWhy, otherLabel, keyShort }) {
  const sets = {
    人間關係: [
      "溝通卡住時，少的通常不是說法，是那句還沒被說出口的為什麼。",
      "對方聽成找麻煩之前，先讓他聽見你在乎。",
      "複述一遍他的意思，比再解釋一次自己的方案更靠近。",
    ],
    事業經營: [
  ...
  if (category === "人間關係" && otherLabel && otherLabel !== "自己") {
    quotes[1] = `先讓${otherLabel}聽見你在乎，再給方案。順序對了，同一句話會變成靠近。`;
  }
...
function localOrganize(rawText) {
  ...
  let themeTitle = {
    人間關係: "溝通不只是說法，而是先釐清背後的原因",
    事業經營: "行動被卡住的真正位置",
    身心狀態: "身體比嘴巴更早說實話",
    覺察: "今天還沒被說完的那一句",
  }[category];

  if (isComm) themeTitle = "溝通不只是說法，而是先釐清背後的原因";
  ...
  const themeInsight = isComm
    ? `今天真正的盲點，不是你「說得不夠好」，是原因還沒出場。${otherLabel}聽到的是你的方案或語氣，聽不到你為什麼要講。先釐清背後的原因，溝通才會從「找麻煩」變成「靠近」。`
    : category === "事業經營"
      ? "今天真正的盲點，不是能力不夠，是下一步還停在抽象。對齊為什麼之後，把行動收到明天做得到的一步，卡住才會鬆開。"
      : category === "身心狀態"
        ? "今天真正的盲點，不是行程排太滿，是身體已經先說實話，嘴巴卻還在撐。允許那一口累，才有下一步。"
        : `今天真正的盲點，不是努力不夠，是有一句話還沒被說清楚。${keyShort ? `「${keyShort}」這句已經碰到了，只差被單獨拿出來看。` : "把為什麼講出口，今天才算被完整接住。"}`;

  const problems = [];

  problems.push({
    title: isComm
      ? "溝通被聽成找麻煩，是因為「為什麼」還沒出場"
      : "少了一句「為什麼」，事情再完整也還是流水帳",
    stars: 5,
    body: keyShort
      ? `你寫了「${keyShort}」。事情被說完了，原因卻還沒站到前面。${isComm ? `${otherLabel}先聽到的是你的方案，不是你的心意。` : "自己也會覺得「我明明有做」，卻沒被接住。"}補一句為什麼，整段對話的溫度會不一樣。`
      : "事情可以寫得很完整，可是少了「為什麼」，它還只是流水帳。先釐清背後的原因，意義才會出現。",
  });

  if (isComm) {
    problems.push({
      title: "話出口之前，少了「先聽懂對方」這一步",
      stars: 4,
      body: `關係裡最容易被當成找麻煩的，不是你的建議，是${otherLabel}還不知道你聽懂了他、也還不知道你為什麼要講。用複述法先對齊，再用一句「我猜你是因為……」確認意圖，關心才不會被聽成指責。`,
    });
  }
  ...
  if (problems.length < 2) {
    problems.push({
      ...
      stars: 4,
      body: "復盤若只停在情緒與事件，明天仍會用同一套方式過。選一個小到不可能失敗的動作——複述一次、先猜意圖、補講為什麼——把今天接到明天。",
    });
  }
  ...
  if (problems.length < 3 && !isComm) {
    problems.push({
      title: "感受有了，意義還缺一句",
      stars: 3,
      body: "你已經碰到今天了。再問一次：這份感覺在提醒你什麼？一句就夠。把它寫成可以帶走的結論，盲點才會真正鬆開。",
    });
  }
  ...
  const reflection = hasWhy
    ...
    : isComm
      ? `當時你急著把話說完，來不及先聽懂${otherLabel}，也來不及問自己為什麼要這樣講。回頭看「${keyShort || "今天這一段"}」，少的不是表達技巧，是先對齊的那一步：複述他的意思、猜他的意圖、再補你的為什麼。`
      : `當時你急著處理事情，來不及問自己為什麼要這樣做。回頭看「${keyShort || "今天這一段"}」，少的不是努力，是先對齊的那一句。現在補上，不是為了翻舊帳，是為了下次開口前先站在同一邊。`;

  const conclusion = isComm
    ? `溝通不只是把話說圓，而是先釐清背後的原因。方案再好，少了一句「為什麼」，也會被${otherLabel}當成找麻煩；先聽懂對方、再講清楚自己在乎什麼，關心才會被聽成心意。`
    : {
  ...
  return {
    ...
    sfm: [
      ...
      },
      {
        ...
        title: "當下的感覺",
        body: isComm
          ? "不是想贏過對方，是害怕自己的在乎被聽成找麻煩。允許這份卡待一會兒，再補一句為什麼。"
...
function localThink(organize, round, selected, reply) {
  const theme = organize?.themeTitle || "溝通不只是說法，而是先釐清背後的原因";
  ...
  const methodActions = [
    {
      label: "複述法",
      detail: `先把${otherFromTheme}的話用你的語言說回去：「所以你的意思是……對嗎？」確認聽懂了，再講你的想法。`,
    },
    {
      label: "先猜測對方意圖",
      detail: `開口前先寫一句：「他可能是因為……才這樣。」猜完再問，而不是先給方案。`,
    },
    {
      label: "補一句為什麼",
      detail: "把方案前面加上：「我說這件事，是因為我在乎……」讓原因站到語氣前面。",
    },
  ];
  ...
  const rounds = [
    {
      title: "深度思考與下一步",
      question: `圍繞「${clipPhrase(theme, 22)}」：如果你只能補一句「為什麼」，那一句會是什麼？`,
      insight: `核心盲點是「${clipPhrase(problem, 22)}」。少的通常不是說法，是動機沒被聽見。你可以先複述對方、猜他的意圖，再把為什麼說清楚——對方才有機會站到你旁邊。`,
      actions: methodActions,
    },
    {
      title: "用方法走一步",
      question: actionHint
        ? `你選了「${actionHint}」。做這一步之前，你最怕${otherFromTheme}聽到的是哪一句？`
        : "如果先用複述法，把對方的話說回去，你猜他最想被聽懂的是哪一句？",
      insight: "找麻煩的感覺，往往來自順序反了：先給方案，再補心意。對調成「先聽懂 → 猜意圖 → 再說為什麼」，同一句話會變成靠近。",
      actions: methodActions,
    },
    {
      title: "聽懂之後再開口",
      ...
      insight: "對外溝通卡住時，內在通常也有一句沒被允許說出口。對自己誠實，對外才講得準。複述法不只用在別人身上，也可以用在自己：把今天的情緒用一句話說回去。",
      actions: [
        { label: "換句話說練習", detail: "把今天最硬的那句，改成對方聽得進去、自己也聽得下去的版本。" },
        { label: "先寫再傳", detail: "傳訊或開口前，先寫「我不是要找麻煩，我是因為在乎」。" },
    ...
    },
    {
      ...
      insight: "抽象的「下次溝通好一點」不會發生。具體的「複述一次」「先猜意圖再開口」「補講一次為什麼」才會發生。",
      actions: methodActions,
    },
    {
      title: "把這一層帶走",
