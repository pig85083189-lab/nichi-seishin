(function (root, factory) {
  ...
  function lastClause(text) {
    const parts = stripEndPunct(cleanText(text)).split(/[，、；]/);
    return String(parts[parts.length - 1] || "").trim();
  }

  function isFinishedThought(text) {
    const core = stripEndPunct(cleanText(text));
    if (!core) return false;
    if (/(因為|所以|如果|但是|但|而且|雖然|就)$/.test(core)) return false;
    const tail = lastClause(core);
    if (/^(但是?|因為|所以|如果|雖然|而且)/.test(tail) && /就[\u4e00-\u9fff]{1,4}了$/.test(tail)) return false;
    return true;
  }
