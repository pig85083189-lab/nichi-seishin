...
function hasReflectionV3Result(value) {
  ...
  return Boolean(data.sourceSig && (data.coreQuote || data.questions.length >= 1));
}
...
function evaluateReflectionV3Quality(result, options) {
  ...
  const issues = [];
  if (!data.coreQuote && data.status !== "empty") issues.push("missing-quote");
  if (data.questions.length < 1 && data.status !== "empty") issues.push("missing-questions");
