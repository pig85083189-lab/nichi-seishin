...
function looksPositiveMoment(line) {
  return /開心|很好|放鬆|輕鬆|笑|幸福|很穩|比較穩|舒服|有選擇|自己(煮|做|選|決定)|沒有立刻/.test(asText(line));
}

function looksConnectionMoment(line) {
  return /媽媽|爸爸|朋友|同事|伴侶|男友|女友|家人|聊天|電話|散步|吃飯|請我喝|陪/.test(asText(line));
}
...
function seeChallenge(candidate, known, raw) {
  ...
  if (seeLooksGeneric(item.statement) || seeLooksNoValue(item.statement) || seeLooksForcedThread(item) || bodyMind.looksSoupBodyMind(item.statement)) failed.push("generic");
