...
先在內部重建這一天，主動找出最值得、且能被原文支持的一個 grounded perspective。
ING 預設：FIND THE MOST VALUABLE GROUNDED PERSPECTIVE AVAILABLE。
不是：DECIDE WHETHER THE USER DESERVES AN INSIGHT。
「我已經知道為什麼難過」≠「沒有更多有用的角度可看」。
...
深度不等於找問題。進步、滋養、價值、選擇、被支持、普通日子的節奏，都可以是有價值的 lens。
優先選最有 grounded 價值的一條；若第一個 lens 不成立，換另一個 lens，不要過早放棄。
對任何有實質內容的日記，通常應至少提出 1 個可被原文支持的 candidate。

【推論層級｜必須遵守】
LEVEL 0 複述原文 → 不夠，不要交。
LEVEL 1  grounded 區分／連結／對比／一步含義 → 預設目標。
LEVEL 2  有原文支持的暫定詮釋 → 允許。
LEVEL 3  隱藏需要／動機／真正來源／心理病因 → 原文沒寫就禁止。
只做 ONE GROUNDED INFERENCE，不要疊加三層心理解釋。
禁止為了聽起來深而寫：「真實來源」「才是…來源」「其實妳需要的是」「被看見的需要落空」。
禁止把「有喝水／很累」膨脹成照顧自己的能力敘事，除非原文明確寫出選擇／練習／第一次。
普通觀察可以保持普通；SPECIFIC + GROUNDED + NOVEL，不是 DEEP-SOUNDING + SPECULATIVE。
...
const SEE_CHALLENGE_SYSTEM = `你要攻擊這些 candidate，不是幫忙圓。目標是留下「有 grounded 價值、且不是複讀」的 core，不是追求 0 個通過。

先把每個 candidate 拆成：
SEE_CORE：有 USER RAW 證據、相對 KNOWN 是新的、不是複讀、能給她至少一個她沒明說的角度
INTERPRETATION_ADDON：心理標籤、沒被支持的因果、身體當心理證據、潛意識、討好、創傷、依附

FACT_CO_OCCURRENCE != KNOWN_CONNECTION
使用者分別寫下 A 和 B，並不是已經說「A 造成 B」。
把兩件獨立事實放在一起看、做 distinction、看節奏或對比，不是 parrot。
只有她自己明確建立的關係才是 KNOWN。
若 CORE 只是並置兩件她沒連起來的事實，且沒有寫成「就是因為 A 所以 B」→ KEEP 或 REVISE，不要當 parrot DROP。
正向 lens（滋養、選擇、被支持、普通日子的節奏）與張力 lens 同等有效。

對 CORE 問：
1. 她看到會不會只說「對啊，我剛剛就寫了」而沒有多一個角度？
2. evidence 是不是 USER RAW？
3. 這會不會讓她從稍微不同的角度看今天？
4. 是不是空話／真但顯而易見／只是「你已經很清楚」？
5. 有沒有過度心理化？是不是 LEVEL 3（真實來源／隱藏需要／真正害怕／其實妳需要）而原文沒寫？
6. 是不是 LABEL_ONLY（更穩定／很在乎關係／正在成長）而沒有說清結構？
7. 是不是把普通疲累／喝水膨脹成「能力／執行力／照顧自己」？

規則：
- CORE 不合格 → DROP。
- CORE 合格、只有 ADDON 不合格 → REVISE：刪掉 addon，只留 core。
- LEVEL 3 沒有原文支持 → DROP 或 REVISE 回 LEVEL 1（事實 vs 詮釋／區分／對比）。
- 明確因果沒被原文支持時，保持暫定語氣（好像／可能／有一個角度）。
- 若全部 DROP，代表真的沒有 grounded 價值；不要因為怕 silence 而硬留 parrot 或硬造深度。
...
function seeChallenge(candidate, known, raw) {
  ...
  if (thinkingCore.looksLevel3Unsupported(item.statement, item.whyItMatters, rawBlob(raw))) failed.push("level3");
  if (thinkingCore.looksStrengthInflation(item.statement, item.whyItMatters, rawBlob(raw))) failed.push("strength-inflation");
  ...
  if (thinkingCore.looksCoOccurrenceOnly(item.statement, item.whyItMatters) && !item.fallbackLens) failed.push("shallow");
  if (
    !item.fallbackLens &&
    !thinkingCore.interpretationHasDepth(item.statement, item.whyItMatters, rawBlob(raw))
  ) {
    failed.push("shallow");
  }
  if (
    item.fallbackLens &&
    !valueLenses.candidateAddsNovelValue(item.statement, raw) &&
    !thinkingCore.interpretationHasDepth(item.statement, item.whyItMatters, rawBlob(raw))
  ) {
    failed.push("shallow");
  }
...
function applySemanticChallenge(candidates, challengeData, known, raw) {
  ...
  candidates.forEach((item) => {
    ...
    if (addonOnlyFailure(row) || verdict === "REVISE") {
      ...
      if (!coreStatement || seeLooksNoValue(coreStatement) || seeLooksEngineLeak(coreStatement) || thinkingCore.looksLevel3Unsupported(coreStatement, coreWhy, rawBlob(raw)) || thinkingCore.looksStrengthInflation(coreStatement, coreWhy, rawBlob(raw))) {
...
有實質內容時，先 exhaust 多個 grounded lens，再考慮空 candidates。
只有極度稀疏、無法負責任推論的輸入，才輸出 {"candidates":[]}`;
}
...
function finalOutputFails(written, selected, known, raw) {
  ...
  if (thinkingCore.looksLevel3Unsupported(statement, support, rawBlob(raw))) return true;
  if (thinkingCore.looksStrengthInflation(statement, support, rawBlob(raw))) return true;
...
async function runSeePipeline(options) {
  ...
  incoming.forEach((row, index) => {
    ...
    if (!judged.ok) {
      const fatal = judged.failed.some((item) =>
        ["known", "parrot", "evidence", "generic", "obvious", "label-only", "shape", "type", "checklist", "level3", "strength-inflation"].includes(item)
      );
  ...
  if (!jsKept.length) {
    ...
    if (valueLenses.isExtremelySparseInput(raw)) {
      return silenceResult(known, { meta, raw });
    }
    if (valueLenses.hasMeaningfulJournalContent(raw)) {
      meta.dropStage = "fallback-exhausted";
    }
    return silenceResult(known, { meta, raw });
  }
  ...
  let selectedFinal = selectedAfterLadder;

  let written = {
    insight: clipSeeText(selectedFinal.statement, 70, 2),
    support: clipSeeText(selectedFinal.whyItMatters, 110, 4),
  };
    ...
    try {
      ...
      if (out && asText(out.insight || out.statement)) {
        const next = {
          ...
          support: clipSeeText(asText(out.support || out.why || selectedFinal.whyItMatters), 110, 4),
        };
        if (discovery.writerIntroducesMeaning(selectedFinal, writerAsCore(selectedFinal, next)) || seeLooksOverreach(next.insight, next.support)) {
