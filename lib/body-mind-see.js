...
function silenceResult(known, extra) {
  ...
  meta.seeTrace = {
    finalStatus: "silence",
    insightPresent: Boolean(copy.insight),
...
function projectSeeOutput(seen) {
  ...
  if (src.status === "silence" || !asText(src.insight)) {
    return { ...silent, _boundary: { insight: "empty", support: "empty" } };
  }
  const cleaned = bodyMind.normalizeBodyMindInsight({ insight: src.insight, support: src.support });
  if (!cleaned.insight) {
    return { ...silent, _boundary: { insight: "empty", support: "empty" } };
  }
  const insightInsp = publicBoundary.inspectPublicText(cleaned.insight, {
    maxChars: publicBoundary.LIMITS.seeInsight,
    fallback: "",
  });
  const supportInsp = publicBoundary.inspectPublicText(cleaned.support || asText(src.support), {
    maxChars: publicBoundary.LIMITS.seeSupport,
    multiline: true,
    fallback: "",
  });
  if (!insightInsp.text || !supportInsp.text) {
    return {
      ...silent,
      _boundary: { insight: insightInsp.status, support: supportInsp.status },
    };
  }
  return {
    insight: insightInsp.text,
    support: supportInsp.text,
    status: "observation",
    ...
    evidence: (Array.isArray(src.evidence) ? src.evidence : [])
      .map((item) => publicBoundary.sanitizePublicText(item, { maxChars: 48, fallback: "" }))
      .filter(Boolean)
      .slice(0, 6),
...
async function runSeePipeline(options) {
  ...
  const boundary = projected._boundary || {};
  delete projected._boundary;
  meta.seeTrace = {
    finalStatus: projected.status,
    insightPresent: Boolean(projected.status !== "silence" && projected.insight),
    boundary,
  };
