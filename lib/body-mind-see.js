...
function projectSeeOutput(seen) {
  ...
  const insight = publicBoundary.sanitizePublicText(cleaned.insight, {
    maxChars: publicBoundary.LIMITS.seeInsight,
    fallback: "",
  });
  const support = publicBoundary.sanitizePublicText(cleaned.support || asText(src.support), {
    maxChars: publicBoundary.LIMITS.seeSupport,
    multiline: true,
    fallback: "",
  });
  if (!insight || !support) return silent;
  return {
    insight,
    support,
    status: "observation",
