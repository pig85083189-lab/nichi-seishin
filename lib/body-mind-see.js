...
function maybeSeedPositiveThread(incoming, raw) {
  ...
  if (careReject) {
    const hasContrast = list.some((row) => {
      ...
      return (
        asText(row && row.type).toUpperCase() === "CONTRAST" ||
        (/感謝|好好說話|陪/.test(text) && /滾出去|搬出去|推開|媽媽/.test(text))
      );
    });
    if (!hasContrast) list.unshift(careReject);
  }
...
function finalOutputFails(written, selected, known, raw) {
  ...
  if (seeLooksNoValue(statement) || seeLooksEngineLeak(`${statement} ${support}`)) return true;
  if (seeLooksOverreach(statement, support) || bodyMind.looksOverPsych(`${statement}${support}`)) return true;
  if (looksUncalibratedCertainty(`${statement} ${support}`)) return true;
...
async function runSeePipeline(options) {
  ...
  let written = {
    insight: clipSeeText(selectedFinal.statement, 120, 3),
    support: clipSeeText(selectedFinal.whyItMatters, 160, 5),
  };
  ...
  if (!canSkipWriter) {
    try {
      ...
      if (out && asText(out.insight || out.statement)) {
        const next = {
          insight: clipSeeText(asText(out.insight || out.statement), 120, 3),
          support: clipSeeText(asText(out.support || out.why || selectedFinal.whyItMatters), 160, 5),
        };
