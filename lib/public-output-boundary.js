...
function sanitizePublicText(text, opts = {}) {
  ...
  if (!cleaned) return opts.allowEmpty === false ? fallback : "";

  if (looksInternalEngineText(cleaned)) return fallback;

  cleaned = stripEngineSentences(cleaned);
  if (!cleaned || looksInternalEngineText(cleaned)) return fallback;

  if (compactChars(cleaned) > maxChars) {
    cleaned = clipToLimit(cleaned, maxChars);
    if (!cleaned || looksInternalEngineText(cleaned)) return fallback;
  }
...
function sanitizePublicQuestion(text, opts = {}) {
  ...
  const cleaned = sanitizePublicText(text, { maxChars, fallback: "" });
  if (!cleaned) return "";
  if (/[？?]\s*$/.test(cleaned)) return cleaned;
  if (/[？?]/.test(asText(text)) && !/[。！]$/.test(cleaned)) return `${cleaned.replace(/[。！]+$/, "")}？`;
  return cleaned;
}
