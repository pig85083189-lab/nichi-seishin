...
const STRUCTURE_BUCKETS = [
  {
    id: "sudden-comply",
    terms: ["答應", "先答應", "留下來", "還是留下", "還是答應", "沒拒絕", "臨時", "突然", "改工作", "加班", "幫忙"],
  },
  {
    id: "boundary-voice",
    terms: ["說不出口", "沒說出來", "沒有立刻答應", "第一次沒", "先停下來", "沒有馬上"],
  },
  {
    id: "knowing-doing",
    terms: ["知道自己不想", "知道不太想", "行動還沒", "還沒做到", "做不到", "知道了卻"],
  },
];
...
不要做診斷或人格標籤。
不要因為同一人物、同一常見詞就給高分。
優先看情境、選擇、反應、決策結構是否相近，不要主要靠名詞重疊。
同一人物、同一詞，但底層情境不同：低分。
不同人物、不同名詞，但面對突然進來的需求時怎麼處理自己，可能相關：可以給 3。
表面文字像、意義不同：低分。
只有過去 AI 假設相似、使用者原文不支持：低分。
不要用「過去 AI 猜測 + 今天 AI 猜測」來證明模式。
...
function structureBucketHits(text) {
  const raw = asText(text);
  return STRUCTURE_BUCKETS.filter((bucket) => bucket.terms.some((term) => raw.includes(term))).map((bucket) => bucket.id);
}

function sharedStructureBuckets(left, right) {
  const a = new Set(structureBucketHits(left));
  const b = new Set(structureBucketHits(right));
...
function scoreStage1(query, candidate, currentDate) {
  ...
  const structureHits = sharedStructureBuckets(q.userRaw, cUser + "\n" + cConfirmed);
  if (structureHits.length) {
    score += Math.min(2.2, structureHits.length * 1.4);
    reasons.push(`structure:${structureHits.join(",")}`);
  }
...
function pickConnectionType(query, candidate, score) {
  ...
  const structureHits = sharedStructureBuckets(q.userRaw, cUser);
  if (hasAction && score >= 3 && (tensionHits.length || personHits.length || positiveHits.length || structureHits.length)) return "prior-success";
  if (structureHits.length && !personHits.length) return "same-choice";
  if (structureHits.length && personHits.length && !tensionHits.length) return "same-choice";
...
function localRerank(query, candidates) {
  return candidates
    .map((candidate) => {
      ...
      const structureHits = sharedStructureBuckets(q.userRaw, layerBlob(candidate.userRaw) + "\n" + layerBlob(candidate.confirmed));
      ...
      else if (structureHits.length && (userHits >= 1 || tensionHits.length)) score = 3;
      else if (personHits.length && !tensionHits.length && !positiveHits.length && !structureHits.length) score = 1;
