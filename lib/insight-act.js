...
const ACT_KINDS = ["ACTION_NOW", "PRACTICE", "OBSERVE"];
...
const NO_CONFIRMED_COPY = {
  line1: "今天沒有一定要帶走的行動。",
  line2: "前面的覺察如果還沒有哪一個特別像你，先不用急著替自己安排下一步。",
};
...
OBSERVE：還不到改行為。可以先觀察。不要寫成穩定模式，不要每天記錄五次。
...
2～3 個已確認覺察若指向同一個下一步，只出 1 條。
最多 0～3 條。寧可 1 條清楚，不要 3 條湊數。

title：短、像下一步名稱。
detail：1～2 句。先說何時做什麼。不要長分析。

只輸出 JSON：
{"decision":"ACTION_NOW|PRACTICE|OBSERVE|NO_ACTION","noActionCopy":{"line1":"","line2":""},"actions":[{"id":"e1","kind":"ACTION_NOW|PRACTICE|OBSERVE","title":"","detail":"","sourceAwarenessIds":["a1"]}]}`;
...
const WHAT = /先停|先確認|先問|問一次|寫下|留意|注意|說一句|說出|不要立刻|不要直接|停一下|核對|留下|只做一件/;
...
async function runActPipeline(options) {
  ...
  if (!confirmed.length) {
    return {
      ...emptyNoAction(ctx, { noActionCopy: NO_CONFIRMED_COPY, meta: { ...meta, calls: 0, blocked: true } }),
      status: "blocked",
      blocked: true,
    };
  }
