...
再決定：給出剛好 3 個具體行動，或 NO_ACTION。
若這份已確認覺察需要帶回生活，請整合今天已確認的覺察、事件、感謝與身心內容，給出 3 個不同且具體的行動。
NO_ACTION 是一等有效結果，但只能在已有 USER_CONFIRMED 覺察後才能判定。
一旦決定給行動，就必須完整給出 3 個，不可只給 1～2 個。

【信任】
只能從 USER_CONFIRMED 05 覺察長出下一步。
未勾選的 05 是假設，禁止當成她已經知道／已經承認。
請整合今天可用來讓行動更貼近生活的內容：已確認覺察、事件、情緒想法、身體覺察、感恩。
未確認覺察不能當成事實或行動理由。
不要說「因為你害怕權威」「我發現你其實……」。

【三個行動槽位｜必須互不重複】
行動 1｜kind=ACTION_NOW：今天／下次當下可做的一小步。先連回今天內容，再說何時做、怎麼做。
行動 2｜kind=PRACTICE：接下來可慢慢練習的方式。先連回今天內容，再說可重複做法。
行動 3｜kind=OBSERVE 或 NOTICE / RECORD / PREPARE：觀察、記錄或事前準備。先連回今天內容。

【品質】
都要小、具體，今天或這週能做到。
每一步都要同時讓她知道：什麼時候做、到時候做什麼。
必須她控制得了、夠小、貼今天情境。
拒絕空泛：愛自己、放鬆一點、相信自己、好好休息、好好溝通、保持覺察、建立界線、勇敢表達。
不替她下心理判斷、不說教。
重大決定（離職／分手／搬家）改成低風險確認／觀察／釐清。
三個行動要分別照顧不同層次：① 當下可做的一小步、② 接下來可練習的方式、③ 可觀察或記錄的提醒。
每一條都必須來自已確認覺察，並可用今天的事件、感謝、身心內容讓它更貼近生活。

leadIn：1～2 句承接，格式接近：
「從你今天寫下的內容裡，我看到你已經知道＿＿＿。接下來不用一次改很多，可以先從這三件小事開始。」

title：短、像下一步名稱。
detail：1～2 句。先連回今天，再說何時做什麼。

只輸出 JSON：
{"decision":"ACTIONS|NO_ACTION","leadIn":"","noActionCopy":{"line1":"","line2":""},"actions":[{"id":"e1","kind":"ACTION_NOW","title":"","detail":"","sourceAwarenessIds":["a1"]},{"id":"e2","kind":"PRACTICE","title":"","detail":"","sourceAwarenessIds":["a1"]},{"id":"e3","kind":"OBSERVE","title":"","detail":"","sourceAwarenessIds":["a1"]}]}`;

const GENERIC = /建立界線|勇敢表達|學習拒絕|更愛自己|好好休息|相信自己|照顧自己的感受|多關注自己的需求|好好愛自己|學習建立界線|好好溝通|放下吧|放鬆一點|愛自己/;
const WHEN = /下次|下一次|今天|今晚|明天|這週|本週|當下|睡前|臨時|再遇到|下回|當.{1,16}(時|的時候)|發生時/;
const WHAT = /先停|先確認|先問|問一次|寫下|留意|注意|說一句|說出|不要立刻|不要直接|停一下|核對|留下|只做一件|記三件|記下來|從哪一刻|準備/;
...
未勾選覺察不要出現在理由裡。
不要新心理學。不要主題句。
若需要行動：必須剛好 3 個、互不重複、分別對應 ACTION_NOW / PRACTICE / OBSERVE（或 NOTICE／RECORD／PREPARE）。
第 1 條是今天／下次當下可做的一小步；第 2 條是接下來可以慢慢練習的方式；第 3 條是可觀察、記錄或準備的提醒。
若當下真的不適合：decision=NO_ACTION、actions=[]。`;
}
...
function pickThreeActSlots(kept) {
  ...
  SLOT_ORDER.forEach((slot, index) => {
    const idx = pool.findIndex((item) => {
      ...
      const key = closeKey(`${item.title}${item.detail}`);
      if (usedText.has(key)) return false;
      return slot.kinds.includes(kind) || (!kind && index === 0);
    });
    if (idx < 0) return;
    const [item] = pool.splice(idx, 1);
    const kind = slot.kinds.includes(asText(item.kind)) ? asText(item.kind) : slot.kinds[0];
    usedText.add(closeKey(`${item.title}${item.detail}`));
    picked.push({ ...item, kind, id: item.id || `e${index + 1}` });
  });
...
function projectActions(actions, ctx, extra) {
  ...
  const leadInRaw = asText(extra && extra.leadIn) || answerEngine.composeActLeadIn(confirmedAwareness(ctx), raw);
  const leadIn = publicBoundary.sanitizePublicText(leadInRaw, {
    maxChars: limits.actLeadIn,
    fallback: "",
  });
  const publicActions = actions
    .slice(0, 3)
    .map((item, index) => {
      const title = publicBoundary.sanitizePublicText(item.title, {
        maxChars: limits.actTitle,
        fallback: "",
      });
      const detail = publicBoundary.sanitizePublicText(item.detail, {
        maxChars: limits.actDetail,
        fallback: "",
      });
      if (!title || !detail) return null;
      return {
    ...
    .filter(Boolean);
  // Preserve exactly-3 contract: if sanitizer wiped a slot, fall back to NO_ACTION rather than leak or partial set.
  if (actions.length === 3 && publicActions.length !== 3) {
    return {
      variant: "execution-v3",
      actVariant: ACT_VARIANT,
      status: "no-action",
      ...
      sourceSig: executionV3.executionV3SourceSig({ ...ctx, ...raw, growVariant: "grow-v1" }),
      noActionCopy: {
        line1: publicBoundary.sanitizePublicText(NO_ACTION_COPY.line1, {
          maxChars: limits.actNoAction,
          fallback: NO_ACTION_COPY.line1,
        }),
        line2: publicBoundary.sanitizePublicText(NO_ACTION_COPY.line2, {
          maxChars: limits.actNoAction,
          fallback: NO_ACTION_COPY.line2,
        }),
      },
    };
  }
  const extraSafe = extra && typeof extra === "object" ? { ...extra } : {};
  delete extraSafe.leadIn;
  delete extraSafe.actions;
  delete extraSafe.noActionCopy;
  delete extraSafe.status;
  return {
    variant: "execution-v3",
    actVariant: ACT_VARIANT,
    status: publicActions.length ? "actions" : "no-action",
    leadIn: publicActions.length ? leadIn : "",
    actions: publicActions,
    options: [],
    selectedIds: [],
    sourceSig: executionV3.executionV3SourceSig({ ...ctx, ...raw, growVariant: "grow-v1" }),
    noActionCopy: publicActions.length
      ? null
      : {
          line1: publicBoundary.sanitizePublicText(NO_ACTION_COPY.line1, {
            maxChars: limits.actNoAction,
...
function attachActTrace(result, ctx, confirmed, meta, extras) {
  const out = result && typeof result === "object" ? result : {};
  const bag = out;
  const nextMeta = { ...(meta || {}), ...(out.meta || {}) };
  nextMeta.actTrace = {
    growStatus: asText(ctx && (ctx.awarenessStatus || ctx.growVariant || "")),
    selectedIdsCount: Array.isArray(confirmed) ? confirmed.length : 0,
    executionAttempted: extras && extras.executionAttempted === false ? false : true,
    finalStatus: asText(bag.status || extras && extras.finalStatus || ""),
    stopHeavyReason: extras && extras.stopHeavyReason ? asText(extras.stopHeavyReason) : null,
    blockedReason: extras && extras.blockedReason ? asText(extras.blockedReason) : null,
  };
  out.meta = nextMeta;
  return out;
}
...
async function runActPipeline(options) {
  ...
  if (!confirmed.length) {
    return attachActTrace(
      emptyBlocked(ctx, { meta: { ...meta, calls: 0, blocked: true, reason: "NO_USER_CONFIRMED_AWARENESS" } }),
      ctx,
      confirmed,
      meta,
      { executionAttempted: false, finalStatus: "blocked", blockedReason: "NO_USER_CONFIRMED_AWARENESS", stopHeavyReason: "selectedIds_empty" }
    );
  }
  ...
  try {
    ...
    return attachActTrace(
      { ...emptyNoAction(ctx, { meta: { ...meta, actError: true } }), empty: true },
      ctx,
      confirmed,
      meta,
      { finalStatus: "no-action", blockedReason: "act_error" }
    );
  }
  ...
  if (slotted.length !== 3) {
    meta.incompleteTriple = true;
    return attachActTrace(
      { ...emptyNoAction(ctx, { meta }), empty: false },
      ctx,
      confirmed,
      meta,
      { finalStatus: "no-action", blockedReason: "incomplete_triple" }
    );
  }

  const leadIn = asText(data && data.leadIn) || answerEngine.composeActLeadIn(confirmed, insightDiscovery.trustRaw(ctx));
  return attachActTrace(
    { ...projectActions(slotted, ctx, { meta, leadIn }), empty: false },
    ctx,
    confirmed,
    meta,
    { finalStatus: "actions" }
