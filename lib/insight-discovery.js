...
function buildKnownByUser(raw) {
  ...
  [raw.thanksText, raw.event, raw.bodyMindText, raw.userAnswer].forEach((block, index) => {
    ...
    // Do NOT treat the whole「我覺得…」clause as a locked conclusion.
    // That over-marks ordinary feelings and blocks exploratory hypotheses.
    const feel = new RegExp(feelRe);
    let match;
    while ((match = feel.exec(text))) {
      const felt = asText(match[1]).replace(/[，。！？].*$/, "");
      if (compactChars(felt) >= 2 && compactChars(felt) <= 12 && /不舒服|難過|開心|生氣|累|痛|悶|空|委屈|刺/.test(felt)) {
        push(`使用者明確表示：${felt}`, "emotion", source);
      }
    }
  });
