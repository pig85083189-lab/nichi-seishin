...
function assertHtcCredentials() {
  if (merchantId() !== "HTC109030010100" || merchantId().length !== 15 || /\s/.test(merchantId())) {
    throw new Error("MerchantID 必須鎖定為 HTC109030010100，且不可有空白");
  }
  if (hashKey() !== "nx1zcPRhfiJS0ECZQcd2XRolrr2P2pKm" || hashKey().length !== 32 || /\s/.test(hashKey())) {
    throw new Error("HashKey 必須鎖定為 HTC 商店 32 碼原文，且不可有空白");
  }
  if (hashIV() !== "PttRZuDsLLO4ZIQC" || hashIV().length !== 16 || /\s/.test(hashIV())) {
    throw new Error("HashIV 必須鎖定為 HTC 商店 16 碼原文，且不可有空白");
  }
}

function mpgKeyBytes() {
  assertHtcCredentials();
...
function logMpgKeyCheck() {
  ...
  console.log("[NewebPay MPG] CREDENTIAL_SOURCE=HARDCODED_HTC_ONLY");
  console.log("[NewebPay MPG] ENV_CREDENTIALS_IGNORED=true");
  console.log(
    "[NewebPay MPG] KEY_CHECK source=HARDCODED_HASH_KEY" +
    ", Key length: " + String(hashKey().length) +
    ", start: " + hashKey().slice(0, 3) + "..." +
    ", end: ..." + hashKey().slice(-3)
  );
  console.log(
    "[NewebPay MPG] IV_CHECK source=HARDCODED_HASH_IV" +
    ", IV length: " + String(hashIV().length) +
    ", start: " + hashIV().slice(0, 3) + "..." +
    ", end: ..." + hashIV().slice(-3)
  );
  console.log(
    "[NewebPay MPG] MID_CHECK source=HARDCODED_MERCHANT_ID" +
    ", length: " + String(merchantId().length) +
    ", value: " + merchantId()
  );
}
...
function encodeItemDesc(value) {
  return phpUrlencode(String(value).replace(/[\r\n\t]/g, "").trim());
}
...
function compactMpgQuery(params) {
  ...
  Object.entries(params).forEach(([key, value]) => {
    ...
    const k = phpUrlencode(String(key).replace(/\s+/g, ""));
    const raw = String(value).replace(/[\r\n\t]/g, "").trim();
    const v = phpUrlencode(raw);
    parts.push(k + "=" + v);
  });
...
function mpgAesEncrypt(plain) {
  const { key, iv } = mpgKeyBytes();
  const cipher = crypto.createCipheriv("aes-256-cbc", key, iv);
  cipher.setAutoPadding(false);
  const padded = addPadding(Buffer.from(String(plain), "utf8"));
  const hex = Buffer.concat([cipher.update(padded), cipher.final()]).toString("hex").toLowerCase();
  return rawTradeInfoHex(hex);
}

function tradeShaSource(tradeInfo) {
  assertHtcCredentials();
  const info = rawTradeInfoHex(tradeInfo);
  if (info !== String(tradeInfo || "").replace(/\s+/g, "").toLowerCase()) {
    throw new Error("TradeInfo 必須是未經過 URL Encode 的純 hex");
  }
...
function tradeSha(tradeInfo) {
  const hex = crypto.createHash("sha256").update(tradeShaSource(tradeInfo), "utf8").digest("hex");
  const sha = hex.toUpperCase();
  if (sha !== sha.toUpperCase() || !/^[0-9A-F]{64}$/.test(sha)) {
    throw new Error("TradeSha 必須是 64 碼大寫 hex");
  }
  return sha;
}

function logOfficialMpgAesVector() {
  const sampleQuery =
  ...
  const cipher = crypto.createCipheriv("aes-256-cbc", key, iv);
  cipher.setAutoPadding(false);
  const padded = addPadding(Buffer.from(sampleQuery, "utf8"));
  const got = Buffer.concat([cipher.update(padded), cipher.final()]).toString("hex").toLowerCase();
  console.log("[NewebPay MPG] OFFICIAL_AES_VECTOR_OK=" + String(got === sampleHex));
  console.log("[NewebPay MPG] AES_PADDING=PKCS7_32 ZERO_PADDING");
}
...
function buildMpgTradeParams(input = {}) {
  ...
  const params = {
    ...
    Version: mpgVersion(),
    LangType: "zh-tw",
    MerchantOrderNo: normalizeMerchantOrderNo(input.orderNo || createOrderNo()),
    Amt: String(mpgAmt()),
  ...
  if (params.MerchantID !== HARDCODED_MERCHANT_ID) {
    throw new Error(`MerchantID 必須是 ${HARDCODED_MERCHANT_ID}，目前是 ${params.MerchantID}`);
  }
  if (params.Amt !== "399") {
...
function normalizeMerchantOrderNo(value) {
  const orderNo = String(value || "").replace(/[^A-Za-z0-9_]/g, "").slice(0, 30);
  if (!/^[A-Za-z0-9_]{1,30}$/.test(orderNo)) {
    throw new Error(`MerchantOrderNo 必須是 1-30 碼英數字或底線，目前是 ${value}`);
  }
  return orderNo;
}
...
function createOrderNo() {
  ...
  return normalizeMerchantOrderNo(`HTC${stamp}${rand}`);
}

function mpgFormFields(tradeParams) {
  assertHtcCredentials();
  const params = { ...tradeParams, MerchantID: merchantId() };
  logMpgKeyCheck();
  ...
  console.log("[NewebPay MPG] MERCHANT_ID=" + String(params.MerchantID));
  console.log("[NewebPay MPG] MERCHANT_ORDER_NO=" + String(params.MerchantOrderNo));
  console.log("[NewebPay MPG] MERCHANT_ORDER_NO_OK=" + String(/^[A-Za-z0-9_]{1,30}$/.test(params.MerchantOrderNo)));
  ...
  if (!/^[0-9a-f]+$/.test(tradeInfo) || tradeInfo.includes("%")) {
    throw new Error("TradeInfo 必須是純小寫 hex，不可 URL Encode");
  }
  if (sha !== sha.toUpperCase() || !/^[0-9A-F]{64}$/.test(sha)) {
    throw new Error("TradeSha 必須是 64 碼大寫 hex");
  }
  if (shaRaw.includes("TradeInfo=")) {
    throw new Error("SHA256 原始字串不可含 TradeInfo=");
  }
  if (shaRaw !== "HashKey=" + hashKey() + "&" + tradeInfo + "&HashIV=" + hashIV()) {
    throw new Error("SHA256 原始字串必須是 HashKey={key}&{TradeInfo}&HashIV={iv}");
  }
  ...
  console.log("[NewebPay MPG] TRADE_SHA_UPPER=true");
  console.log("[NewebPay MPG] KEY_CHARS=" + String(hashKey().length) + " IV_CHARS=" + String(hashIV().length));
  console.log("[NewebPay MPG] KEY_HAS_WHITESPACE=" + String(/\s/.test(hashKey() + hashIV() + merchantId())));
...
function buildPeriodTradeParams(input = {}) {
  ...
  const params = {
    ...
    LangType: "zh-Tw",
    MerOrderNo: normalizeMerchantOrderNo(input.orderNo || createOrderNo()),
...
function periodFormBody(fields) {
  ...
  official.MerchantID_ = merchantId();
