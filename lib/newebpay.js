...
function merchantIdNames() {
  const extra = isProduction()
    ? ["NEWEBPAY_MERCHANT_ID_PROD"]
    : ["NEWEBPAY_MERCHANT_ID_TEST"];
  return ["NEWEBPAY_MPG_MERCHANT_ID", ...extra, "NEWEBPAY_MERCHANT_ID", "MerchantID", "MERCHANT_ID", "NEWEBPAY_MID"];
}

function hashKeyNames() {
  const extra = isProduction()
    ? ["NEWEBPAY_HASH_KEY_PROD"]
    : ["NEWEBPAY_HASH_KEY_TEST"];
  return ["NEWEBPAY_MPG_HASH_KEY", ...extra, "NEWEBPAY_HASH_KEY", "HashKey", "HASH_KEY", "NEWEBPAY_HashKey"];
}

function hashIvNames() {
  const extra = isProduction()
    ? ["NEWEBPAY_HASH_IV_PROD"]
    : ["NEWEBPAY_HASH_IV_TEST"];
  return ["NEWEBPAY_MPG_HASH_IV", ...extra, "NEWEBPAY_HASH_IV", "HashIV", "HASH_IV", "NEWEBPAY_HashIV"];
}

function merchantIdPick() {
  return pickEnv(merchantIdNames());
}

function hashKeyPick() {
  return pickEnv(hashKeyNames());
}

function hashIvPick() {
  return pickEnv(hashIvNames());
}

function merchantId() {
  return merchantIdPick().value;
}

function hashKey() {
  return hashKeyPick().value;
}

function hashIV() {
  return hashIvPick().value;
}
...
function logMpgKeyCheck() {
  ...
  console.log("[NewebPay MPG] ENV_NAME=" + (newebpayEnvName() || "(unset→prod)") + " gateway=" + mpgGateway());
...
function sanitizeTradeInfo(hex) {
...
function mpgAesEncrypt(plain) {
  ...
  cipher.setAutoPadding(false);
  const padded = addPadding(Buffer.from(String(plain), "utf8"));
  return sanitizeTradeInfo(Buffer.concat([cipher.update(padded), cipher.final()]).toString("hex"));
}

function tradeShaSource(tradeInfo) {
  return `HashKey=${hashKey()}&${sanitizeTradeInfo(tradeInfo)}&HashIV=${hashIV()}`;
}
...
function mpgFormFields(tradeParams) {
  ...
  logMpgKeyCheck();
  const { query, tradeInfo, tradeSha: sha, shaRaw } = encryptTradeInfo(params);
  let roundtrip = "";
  try {
