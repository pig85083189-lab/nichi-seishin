...
function assertHtcCredentials() {
  if (merchantId() !== HARDCODED_MERCHANT_ID) {
    throw new Error("MerchantID 必須鎖定為 HTC109030010100");
  }
  if (hashKey() !== HARDCODED_HASH_KEY) {
    throw new Error("HashKey 必須鎖定為 HTC 商店金鑰");
  }
  if (hashIV() !== HARDCODED_HASH_IV) {
    throw new Error("HashIV 必須鎖定為 HTC 商店 IV");
  }
}
