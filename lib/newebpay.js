...
const OFFICIAL_PERIOD_QUERY_KEYS = [
  "RespondType",
  "TimeStamp",
  "Version",
  "LangType",
...
function inspectPeriodQuery(query) {
  ...
  const keys = text.split("&").map((pair) => pair.split("=")[0]).filter(Boolean);
  const officialPrefix = keys.slice(0, OFFICIAL_PERIOD_QUERY_KEYS.length);
  return {
    keys: keys.join(","),
    officialOrderOk: officialPrefix.join(",") === OFFICIAL_PERIOD_QUERY_KEYS.join(","),
    startsRespondTypeJson: text.startsWith("RespondType=JSON&"),
    hasVersion15: /(?:^|&)Version=1\.5(?:&|$)/.test(text),
...
function logPeriodQuery(params, query) {
  ...
  if (!inspect.officialOrderOk || !inspect.hasVersion15 || inspect.hasLowercaseVersion || !inspect.startsRespondTypeJson) {
    throw new Error("Period query 與官方 1.5 欄位順序／大小寫不一致");
  }
}
...
async function probePeriodGateway(fields) {
  ...
  const body = periodFormBody(official);
