...
function merchantId() {
  return firstEnv(["NEWEBPAY_MPG_MERCHANT_ID", "NEWEBPAY_MERCHANT_ID", "MerchantID", "MERCHANT_ID", "NEWEBPAY_MID"]);
}

function hashKey() {
  return firstEnv(["NEWEBPAY_MPG_HASH_KEY", "NEWEBPAY_HASH_KEY", "HashKey", "HASH_KEY", "NEWEBPAY_HashKey"]).replace(/\s+/g, "");
}

function hashIV() {
  return firstEnv(["NEWEBPAY_MPG_HASH_IV", "NEWEBPAY_HASH_IV", "HashIV", "HASH_IV", "NEWEBPAY_HashIV"]).replace(/\s+/g, "");
}
...
function mpgFormFields(tradeParams) {
  ...
  console.log("[NewebPay MPG] KEY_SOURCE_CHARS=" + JSON.stringify({
