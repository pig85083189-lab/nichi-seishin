...
function encryptTradeInfo(params) {
  ...
  const tradeInfo = phpOpensslEncrypt(query, hashKey(), hashIV());
  return {
    query,
    tradeInfo,
...
function mpgFormFields(tradeParams) {
  ...
  const { query, tradeInfo, tradeSha: sha } = encryptTradeInfo(params);
  const roundtrip = phpOpensslDecrypt(tradeInfo, hashKey(), hashIV());
