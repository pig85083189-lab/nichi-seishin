...
const EXECUTION_V3_SYSTEM = `你是「進行式 ING」的下一步整理者。
...
function normalizeExecutionV3Actions(raw) {
  ...
  return list
    .map((item, index) => {
      ...
      return { id: String((item && item.id) || `e${index + 1}`), title, detail, text: title };
    })
    .filter(Boolean)
    .slice(0, 3);
}
...
function hasExecutionV3Result(value) {
  return normalizeExecutionV3(value).actions.length >= 3;
}
...
function executionV3Ready(ctx) {
  ...
  return Boolean(
    compactChars(data.thanksText || data.thanks) >= 2 &&
      ...
      (generated.length >= 3 || compactChars(data.bodyMindText || data.bodyNote) >= 6)
  );
}
...
【03 身心覺察原文】
${compactLine(ctx.bodyMindText || ctx.bodyNote, 300) || "未寫"}
