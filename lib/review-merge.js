(function (root, factory) {
  ...
  function executionChoiceOptionExtras(item, title) {
    ...
    const actKind = String(item.actKind || item.kind || "").trim();
    if (/^(ACTION_NOW|PRACTICE|OBSERVE)$/.test(actKind)) {
      extras.actKind = actKind;
      extras.kind = extras.kind || (actKind === "OBSERVE" ? "observe" : actKind === "PRACTICE" ? "experiment" : "act");
    }
    const kind = normalizeExecutionChoiceKind(item.kind || item.type);
    if (kind && !extras.kind) extras.kind = kind;
    const horizon = normalizeExecutionChoiceHorizon(item.horizon || item.when);
    if (horizon) extras.horizon = horizon;
    if (Array.isArray(item.sourceAwarenessIds)) {
      extras.sourceAwarenessIds = item.sourceAwarenessIds.map((id) => String(id || "").trim()).filter(Boolean).slice(0, 3);
    }
    return extras;
  }
  ...
  function isActExecutionBag(data) {
    return data.actVariant === "act-v1" || data.status === "actions" || data.status === "no-action";
  }

  function hasMeaningfulExecutionChoices(value) {
    if (!isPlainObject(value)) return false;
    const bag = normalizeExecutionChoiceBag(value);
    if (isActExecutionBag(bag) && bag.sourceSig) return true;
    return (
  ...
  function mergeExecutionChoiceBags(older, newer) {
    ...
    const preferActB = isActExecutionBag(b) && b.sourceSig && (Date.parse(b.generatedAt || "") || 0) >= (Date.parse(a.generatedAt || "") || 0);
    const actWinner = preferActB ? b : isActExecutionBag(a) && !isActExecutionBag(b) ? a : null;
    if (actWinner && isActExecutionBag(b) && b.sourceSig) {
      return {
        ...b,
        selectedIds,
        selectedId: selectedIds[0] || "",
    ...
    return {
      ...
      actVariant: b.actVariant || a.actVariant,
      status: b.status || a.status,
      sourceSig: (b.options.length || isActExecutionBag(b) ? b.sourceSig : "") || a.sourceSig || b.sourceSig,
      options,
      selectedId: selectedIds[0] || "",
      selectedIds,
      ...
      deep: mergeExecDeep(a.deep, b.deep),
      noActionCopy: b.noActionCopy || a.noActionCopy,
      leadIn: b.leadIn || a.leadIn || "",
    };
  }
  ...
  function preserveBodyMindText(value, multiline) {
  ...
  function normalizeBodyMind(raw) {
    ...
    return {
      text: preserveBodyMindText(src.text || src.note || "", false),
      insight: preserveBodyMindText(src.insight || "", false),
      support: preserveBodyMindText(src.support || "", true),
      generatedAt: String(src.generatedAt || "").trim(),
      ...
      thinkingCore: src.thinkingCore && typeof src.thinkingCore === "object" ? src.thinkingCore : null,
  ...
  function mergeBodyMind(older, newer) {
    ...
    return {
      ...
      thinkingCore: b.thinkingCore || a.thinkingCore,
