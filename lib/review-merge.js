(function (root, factory) {
  ...
  function normalizeReflectionExtensionQuestions(raw) {
    ...
    return list
      .map((item, index) => {
        ...
        return text ? { id: String((item && item.id) || `eq${index + 1}`), text } : null;
      })
      .filter(Boolean)
      .slice(0, 3);
  }
  ...
  function isExtensionRoundCompleted(round) {
    if (!isPlainObject(round)) return false;
    const answerChars = String(round.answer || "").replace(/\s+/g, "").trim().length;
    return Boolean(
  ...
  function normalizeReflectionExtensionRound(raw, index) {
    ...
    const completed = Boolean(deepConclusion && completedAt && selectedQuestionId && String(answer).replace(/\s+/g, "").trim().length >= 8);
  ...
  function hasMeaningfulReflectionExtension(raw) {
    ...
    return data.rounds.some((item) => item.id || item.questions.length || item.answer || item.deepConclusion || item.selectedQuestionId);
  }
  ...
  function mergeReflectionExtension(older, newer) {
    if (!hasMeaningfulReflectionExtension(newer)) {
      return hasMeaningfulReflectionExtension(older) ? normalizeReflectionExtension(older) : emptyReflectionExtension();
    }
    if (!hasMeaningfulReflectionExtension(older)) return normalizeReflectionExtension(newer);
  ...
  function emptyBodyMind() {
    return { text: "", insight: "", support: "", generatedAt: "", sig: "", internalDebug: null };
  }
  ...
  function mergeAwarenessV3(older, newer) {
    ...
    if (!b.items.length) return a.items.length ? a : b;
    if (!a.items.length) return b;
    const stamp = (value) => {
    ...
    return {
      ...winner,
      selectedIds,
      observationCue: pickObservationCue(a, b, selectedIds, winner.items),
    };
  }
