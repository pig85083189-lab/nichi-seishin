...
async function supabaseRest(path, options = {}) {
  ...
  const response = await fetch(`${supabaseUrl()}/rest/v1/${path}`, {
    ...
    headers: {
      apikey: key,
      Authorization: `Bearer ${supabaseServiceKey() || options.userToken || key}`,
...
async function saveSupabaseUserData(userId, bundle, extra = {}) {
  const id = String(userId || "").trim();
  if (!id || !supabaseConfigured() || !(supabaseServiceKey() || extra.userToken || restKey())) return false;
  const payload = {
    user_id: id,
    email: extra.email || "",
  ...
  const { ok, status, data } = await supabaseRest(`${TABLE}?on_conflict=user_id`, {
    method: "POST",
...
function isEntitled(row) {
  if (!row) return false;
  return isPaid(row) || isTrialActive(row);
}
...
async function ensureTrial(user) {
  ...
  if (!existing) {
    ...
    return upsertSubscription({
      ...
      status: "trialing",
      plan: "monthly",
      amount: Number(process.env.NEWEBPAY_PERIOD_AMT || process.env.NEWEBPAY_AMT || 599) || 599,
  ...
  if (!isPaid(existing)) {
    const started = existing.trial_started_at || existing.created_at;
    if (started) {
      const correctEnd = trialEndFromStart(started);
      const currentEnd = Date.parse(existing.trial_ends_at);
      const nextEnd = Date.parse(correctEnd);
      if (Number.isFinite(nextEnd) && (!Number.isFinite(currentEnd) || nextEnd > currentEnd)) {
        patch.trial_ends_at = correctEnd;
      }
    }
