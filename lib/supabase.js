...
function isInternal(row) {
  return String((row && row.access_type) || "").trim().toLowerCase() === "internal";
}

async function isInternalUser(userId) {
  const id = String(userId || "").trim();
  const admin = getAdminClient();
  if (!id || !admin) return false;
  try {
    const { data, error } = await fromPublic(admin, INTERNAL_TABLE)
      .select("user_id,access_type")
      .eq("user_id", id)
      .maybeSingle();
    if (error || !data) return false;
    return String(data.access_type || "internal").trim().toLowerCase() === "internal";
  } catch (error) {
...
async function listInternalUserIds() {
  ...
  if (!admin) return [];
  try {
    const { data, error } = await fromPublic(admin, INTERNAL_TABLE).select("user_id,access_type");
    if (error || !Array.isArray(data)) return [];
    return data
      .filter((row) => String((row && row.access_type) || "internal").trim().toLowerCase() === "internal")
...
async function withInternalAccess(row, userId) {
  const id = String(userId || (row && row.user_id) || "").trim();
  const internal = await isInternalUser(id);
  if (internal) {
    return { ...(row || { user_id: id }), access_type: "internal" };
  }
...
async function ensureTrial(user) {
  ...
  const internal = await isInternalUser(user.id);
  const now = new Date();
  if (!existing) {
    const started = now.toISOString();
    const ends = trialEndFromStart(started);
    console.log("ensureTrial insert", user.id, "trialDays", TRIAL_DAYS);
    ...
    return withInternalAccess(created, user.id);
  }
  const patch = {};
  ...
  if (!Object.keys(patch).length) return withInternalAccess(existing, user.id);
  const updated = (await patchSubscription(user.id, patch)) || { ...existing, ...patch };
  return withInternalAccess(updated, user.id);
}
