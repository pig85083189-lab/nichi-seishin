...
async function ensureTrial(user) {
  ...
  if (!existing) {
    ...
    return upsertSubscription({
      ...
      status: "trialing",
      plan: "monthly",
      amount: Number(process.env.NEWEBPAY_PERIOD_AMT || process.env.NEWEBPAY_AMT || 399) || 399,
  ...
  if (!isPaid(existing)) {
    ...
    if (!OPEN_STATUSES.has(nextRow.status) && nextRow.status !== "cancelled" && !isTrialActive(nextRow) && nextRow.status !== "expired") {
