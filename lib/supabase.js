...
async function listInternalUserIds() {
  const fromEnv = parseInternalAllowlist().ids;
  const admin = getAdminClient();
  if (!admin) return fromEnv;
  try {
    ...
    const fromTable = !result.error && Array.isArray(result.data)
      ? result.data
          .filter((row) => isInternalAccessType((row && row.access_type) || "internal"))
          .map((row) => String(row.user_id || "").trim())
          .filter(Boolean)
      : [];
    return [...new Set([...fromTable, ...fromEnv])];
  } catch (error) {
...
async function ensureTrial(user) {
  ...
  if (!internal && !isPaid(existing)) {
    // 既有列的 trial_ends_at 不重算。TRIAL_DAYS 變更只影響新 insert，避免舊 30 天 trial 被縮短或過期帳號被重送 7 天。
