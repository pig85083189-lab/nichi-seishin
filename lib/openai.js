...
function buildOpenAIPayload(messages, opts, model) {
  ...
  if (isGpt5Family(model)) {
    if (opts.skipReasoning !== true) {
      const effort = String((opts && opts.effort) || "medium").trim() || "medium";
...
async function callClaude(messages, opts) {
  ...
  try {
    ...
    return finishWithMeta(
      parsed,
      { usage: normalizeUsage(data.usage), model, provider: "anthropic" },
      opts
    );
  } catch (error) {
...
async function callOpenAIHttp(messages, opts) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (!opts._retried && isGpt5Family(model) && response.status === 400 && /temperature|reasoning_effort/i.test(msg)) {
    ...
    return finishWithMeta(
      parsed,
      { usage: normalizeUsage(data.usage), model: data.model || model, provider: "openai" },
      opts
    );
  } catch (error) {
