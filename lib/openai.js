...
async function callClaude(messages, opts) {
  ...
  try {
    ...
    return parseAiJson(text, { rejectPartial: opts.rejectPartial !== false });
  } catch (error) {
...
async function callOpenAI(messages, options = {}) {
  ...
  try {
    ...
    return parseAiJson(data?.choices?.[0]?.message?.content || "", { rejectPartial: opts.rejectPartial !== false });
  } catch (error) {
    if (error?.name === "AbortError") throw abortError();
    throw error;
  } finally {
