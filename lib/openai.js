...
function attachProviderDebug(error, extra) {
  if (!error || typeof error !== "object") return error;
  const src = extra && typeof extra === "object" ? extra : {};
  const usage = src.usage && typeof src.usage === "object" ? src.usage : null;
  error.providerDebug = {
    provider: src.provider || "",
    model: src.model || "",
    reasoningEffort: src.reasoningEffort || "",
    httpStatus: src.httpStatus != null ? src.httpStatus : error.status != null ? error.status : null,
    finishReason: src.finishReason || "",
    stopReason: src.stopReason || "",
    usage,
    code: src.code || error.code || "",
  };
  return error;
}

function abortError(provider, extra) {
  const viaOpenAI = provider === "openai" || (!provider && !usesClaude());
  const error = new Error(viaOpenAI ? "OpenAI 逾時" : "Claude 逾時");
  error.status = 504;
  error.name = "AbortError";
  error.code = "TIMEOUT";
  return attachProviderDebug(error, {
    provider: viaOpenAI ? "openai" : "anthropic",
    httpStatus: 504,
    code: "TIMEOUT",
    ...(extra || {}),
  });
...
function normalizeUsage(raw) {
  ...
  const details = data.completion_tokens_details || data.output_tokens_details || {};
  ...
  const reasoning = Number(details.reasoning_tokens || data.reasoning_tokens || 0) || 0;
...
async function callClaude(messages, opts) {
  ...
  try {
    ...
    const debugBase = {
      ...
      stopReason: data && data.stop_reason ? String(data.stop_reason) : "",
    ...
    if (!response.ok) {
      ...
      throw attachProviderDebug(error, debugBase);
    }
    ...
    try {
      ...
      return finishWithMeta(
        parsed,
        {
          usage: debugBase.usage,
          model,
          provider: "anthropic",
          reasoningEffort: debugBase.reasoningEffort,
          endpoint: "anthropic.messages",
          httpStatus: response.status,
          stopReason: debugBase.stopReason,
          finishReason: debugBase.stopReason,
        },
        opts
      );
    } catch (error) {
      throw attachProviderDebug(error, { ...debugBase, code: error && error.code });
    }
  } catch (error) {
    if (error?.name === "AbortError") throw abortError("anthropic", { model });
    throw error;
  } finally {
    clearTimeout(timer);
  }
...
async function callOpenAIHttp(messages, opts) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const error = new Error(msg);
      error.status = response.status;
      throw attachProviderDebug(error, {
        provider: "openai",
        model,
    ...
    const finishReason = data && data.choices && data.choices[0] ? String(data.choices[0].finish_reason || "") : "";
    ...
    if (error?.name === "AbortError") throw abortError("openai", { model });
    throw error;
  } finally {
    clearTimeout(timer);
  }
