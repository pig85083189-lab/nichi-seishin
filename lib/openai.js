...
function parseAiJson(raw, opts = {}) {
  ...
  if (opts.rejectPartial) throw incompleteAiError();
...
async function callClaude(messages, opts) {
  ...
  try {
    ...
    return parseAiJson(text, { rejectPartial: Boolean(opts.rejectPartial) });
  } catch (error) {
...
async function callOpenAI(messages, options = {}) {
  ...
  // 有 ANTHROPIC_API_KEY 時一律優先 Claude Haiku；沒有才走 OpenAI / OpenRouter。
  ...
  try {
    ...
    return parseAiJson(data?.choices?.[0]?.message?.content || "", { rejectPartial: Boolean(opts.rejectPartial) });
  } catch (error) {
    if (error?.name === "AbortError") throw abortError();
    throw error;
  } finally {
...
module.exports = { getApiKey, getModel, getProvider, usesClaude, callOpenAI, parseAiJson, incompleteAiError };
