...
不要新開一個心理分析。不要給 action。不要三句同一意思。
三個 item 應該是三個不同、但今天真的有證據的「我看見」。
...
function normalizeAwarenessV3Result(raw, ctx) {
  const items = normalizeAwarenessV3Items(raw).filter(
...
每個 item：title（我要覺察什麼）＋ 第一人稱 text。白話。不要抽象詞。三句不同，而且今天真的有證據。
