(function (root, factory) {
  ...
  function scoreTitle(text) {
    const n = compactLen(text);
    if (!n) return 0;
    if (n >= TITLE_MIN && n <= TITLE_MAX) return 300 - Math.abs(20 - n);
    if (n >= TITLE_SOFT_MIN && n < TITLE_MIN) return 120 + n;
    return 0;
  }
  ...
  function buildHistoryDisplayTitle(review) {
    ...
    candidates.forEach((text) => {
      completeClauses(text).forEach((clause) => {
        const score = scoreTitle(clause);
        if (score > bestScore) {
          best = clause;
          bestScore = score;
        }
