(function (root, factory) {
  ...
  function completeClauses(text) {
    ...
    if (!sentence) return [];
    const found = [];
    if (compactLen(sentence) <= TITLE_MAX) found.push(sentence);
    const parts = sentence.split(/[，、；：,:]/).map((item) => item.trim()).filter(Boolean);
    let acc = "";
    parts.forEach((part) => {
      ...
      if (compactLen(part) <= TITLE_MAX) found.push(part);
      acc = compactLen(part) <= TITLE_MAX ? part : "";
    });
    return [...new Set(found.map(cleanTitleText).filter(Boolean))];
  }
  ...
  function scoreQuoteCandidate(text, sourceWeight) {
    ...
    if (!raw || n < TITLE_SOFT_MIN || n > TITLE_MAX) return 0;
    ...
    if (n >= TITLE_MIN && n <= 26) score += 40;
    else if (n <= TITLE_MAX) score += 16;
  ...
  function bestExistingQuote(review) {
    ...
    collectInsightSources(review).forEach((item) => {
      ...
      if (compactLen(item.text) <= TITLE_MAX) clauses.push(cleanTitleText(item.text));
      clauses.forEach((clause) => {
        if (!canReuseSourceQuote(item.key, clause)) return;
        let score = scoreQuoteCandidate(clause, insightSourceWeight(item.key));
        if (clause === cleanTitleText(item.text) && compactLen(clause) <= TITLE_MAX) score += 50;
        if (score > bestScore) {
