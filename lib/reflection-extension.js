...
2. 使用者今天原文
3. 第一輪 selected question 與 deepConclusion
4. 其他 AI 推論最後
不要忽略她親自回答，又自己跑回原本假設。
...
NOT YET EXPLORED：尚未回答的 contradiction、尚未拆開的 trade-off、尚未釐清的 value、尚未看見的 alternative、尚未思考的 boundary / choice。

第二輪只能從 NOT YET EXPLORED 產生問題。
第二輪也要有 ONE NEXT CORE THREAD，而且必須是第一輪的延伸，不是平行亂跳。
不要再問她已經回答過的事。
...
function evaluateExtensionAskQuality(result, options) {
  ...
  texts.forEach((text, index) => {
    ...
    if (looksOverloadedQuestion(text) || chars > 78) issues.push(`q${index + 1}-too-long`);
...
這是 NEXT LAYER，不是再給一組問題。
先看第一次已經看見什麼、她親自回答了什麼，再只從尚未探索的方向往下。

第一輪核心：${compactLine(prior.coreThread, 80) || "無"}
第一輪選中｜ALREADY EXPLORED：${compactLine(prior.selectedQuestion || prior.selectedQuestionText, 200) || "無"}
第一輪她親自回答｜最高證據：
...
第二輪必須有 ONE NEXT CORE THREAD，而且是第一輪的延伸。
不要再問她已經回答過的事。不要把結論換句再問一次。`;
}
...
function reflectionExtensionAskUserPrompt(body) {
  ...
  const intro = prior
    ? "這是第二次延伸。不要重新從今天 01～04 開始分析。先承接第一輪的 selected question、user answer 與 deepConclusion，只從尚未探索的方向再往下一層。三題必須圍繞 ONE NEXT CORE THREAD。每題 1 句、約 30～70 字。不要訪談。不要給結論。不要做 05／06。只讀今天。"
