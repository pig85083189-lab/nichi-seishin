...
再從這個核心產生三個不同角度的洞察。
每一個先給 title + insight；再決定 question。
Extension 不是把問題問得更深。
它的任務是：從使用者剛剛的回答裡，找到一個新的值得看見的東西。
...
【問題必須有前因，而且白話】
每一題內部必須有 title + insight + question，寫成自然完整的內容。
先讓她看見一個新的東西，再問。
不可以突然只丟一個看起來很深的問題，也不可以只是把她的回答換句話說再問一次。
...
不是把 Round 1 question 改寫。
任務：找出 Round 1 answer 裡的新矛盾、新模式、新假設、還沒被注意的地方。
然後：title + insight + question。
如果沒有真正新的 layer：不要假裝深。可以問一個簡單但有價值的問題。
不能只是同主題另一個角度。不能重新回到今天事件表面。不能 solution jump。

例如第一輪她回答「好像要有人提醒我，那個不確定才比較容易消失」：
不要問：「你是否依賴外部肯定建立內在篤定？」
也不要只是重述：「你說只要能掌控就好。那真正讓你安心的是掌控還是答案？」
要先給看見，再問一個答案不在原文裡的問題。
...
只輸出 JSON。繁體中文。
{
  "coreThread": "",
  "items": [
    { "id": "eq1", "title": "", "insight": "", "question": "" },
    { "id": "eq2", "title": "", "insight": "", "question": "" },
    { "id": "eq3", "title": "", "insight": "", "question": "" }
  ]
}`;
