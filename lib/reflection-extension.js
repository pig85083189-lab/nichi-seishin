...
function reflectionExtensionAskUserPrompt(body) {
  ...
  const intro = prior
    ? "這是第二次延伸。最高權重是第一輪 user answer。先找出回答裡已經確認、但還沒真正解開的張力，再往 NEXT LAYER。不要 PARALLEL ANGLE。不要重新從今天 01～04 找新主題。不要跳到怎麼做／怎麼讓別人知道。三題必須圍繞 ONE NEXT CORE THREAD。每題 1 句、約 25～60 字。不要訪談。不要給結論。不要做 05／06。只讀今天。不要讀過往日期。"
    : pastBlock
      ? "請先找出今天最值得繼續往下的 ONE CORE THREAD。今天是主體。相關過往只在能增加新理解角度時當輔助。不要被歷史蓋過。不要把三篇過往各問一題。不要寫你總是／你之前就曾經。每題 1 句、約 30～70 字。不要訪談。不要給結論。不要做 05／06。"
