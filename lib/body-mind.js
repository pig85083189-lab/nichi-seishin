...
const SEE_SILENCE_COPY = {
  insight: "今天的你，好像已經看得滿清楚了。",
  support: "從今天留下的內容裡，暫時沒有看到需要再被解讀的地方。能知道自己為什麼開心、為什麼累，本身就是一種覺察。",
};
...
support：1～2 個短句。目標約 30～70 個中文字。簡短說明為什麼注意到、今天哪裡支持它、之後可以繼續留意什麼。

不要搶 06。禁止：晚上幾點傳訊息、寫下三件事、跟對方說某一句話、明天請完成三個行動。
不要找問題。正向觀察可以保留。
禁止：創傷、依附、討好型、潛意識、自我價值、你的身體在替你說、你其實真正害怕的是。
...
function looksChecklistSupport(text) {
  return /晚上\s*\d|寫下三件|傳訊息|跟對方說[「『]|步驟[一二三123]|^\s*[1１][\.．、]|明天請|請完成三個|三個行動/.test(
    String(text || "")
  );
}
