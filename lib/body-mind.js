...
你只負責把已經通過的 03 SEE Core 寫成給使用者看的覺察文案。
03 是 SEE｜看見。核心問題：今天有什麼，是她自己還沒注意到，但值得她知道的？
看完整天的 USER RAW，不要只複述身心 textarea。
輸出必須和 Core 語意等價。不能新增意義。語氣溫柔、具體、穩定、有條理。

先具體承接她今天寫的內容，再整理 2～3 個發現；可有一個保留不確定性的延伸理解。
不要為了完整硬湊感恩、事件、身體三者；有自然連結才寫，沒有就分著看。
不要寫成 04 深度思考，不要寫成 06 行動教練。不要搶 06。
不要找問題。正向觀察可以保留。
禁止：創傷、依附、討好型、潛意識、自我價值——若只是假設，必須用「可能／也許／有沒有一種可能」標成假說。
禁止把假說寫成「真正的問題就是／這證明妳／妳一定是」。
禁止訪談式追問。不要空泛療癒金句（好好愛自己／相信自己）。
感恩清單只能整理她原文裡真的有的感謝；沒有就回空陣列，不要硬編。

只輸出 JSON：
{
  "theme": "一句白話今日主題",
  "sectionTitle": "一句描述今天核心的話",
  "reception": "一小段承接她今天實際寫的內容",
  "findings": ["具體發現1", "具體發現2"],
  "extension": "一個保留不確定性的延伸理解",
  "conclusion": "一句穩定、有力量、不浮誇的收束",
  "quotes": ["可複製短句1", "可複製短句2"],
  "gratitude": ["原文整理的感恩1"]
}
繁體中文`;
...
function normalizeBodyMindInsight(raw) {
  ...
  let insight = preserveSeeText(src.insight || src.line || "").replace(/^「|」$/g, "");
  let support = preserveSeeText(src.support || src.note || "", { multiline: true });
  const formatted = /【核心結論】|【今日金句】|^主題[：:]/.test(`${insight}\n${support}`);
  if (!formatted) {
    insight = stripInterviewOnlyQuestions(insight);
    support = stripInterviewOnlyQuestions(support);
  }
  if (looksSoupBodyMind(insight) || looksOverPsych(insight) || looksMedicalCause(insight)) insight = "";
  if (!formatted && (looksSoupBodyMind(support) || looksChecklistSupport(support))) support = "";
...
function evaluateBodyMindQuality(result, options) {
  ...
  if (!insight) issues.push("missing-insight");
  if (!support) issues.push("missing-support");
  if (!formatted && looksRestate(userText, insight)) issues.push("restate");
  ...
  if (!formatted && looksChecklistSupport(support)) issues.push("support-is-checklist");
  if (!formatted && looksQuestionOutput(blob)) issues.push("asked-question");
  ...
  if (looksStackedInsight(insight) && !formatted) issues.push("stacked-insight");
  if (!formatted && looksLongBodyMind(insight, 80)) issues.push("insight-too-long");
  if (!formatted && (countBodyMindSentences(support) > 4 || compactBodyMindChars(support) > 120)) issues.push("support-too-long");
