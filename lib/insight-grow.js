...
const EMPTY_COPY = {
  line1: "今天不一定要再多加一個覺察標籤。",
  line2: "前面若已有一個真正像你的地方，先停在那裡就好。",
};
...
目標：提供 1～3 個生活中做得到、可讓她自己勾選的方向。不命令、不說教。
不要把 AI 推測轉成她的結論。
...
不要每個方向各出一條。目標 1～3。寧可 1 條有力，不要 3 條平庸。0 也成立。
ALREADY_DONE 可以是唯一結果。不要每天只找自己哪裡不夠。

材料是：today raw + USER ANSWER + 現行解釋（可能已被 04 回答改過）。
不要證明 03 當初的假設。
如果 USER ANSWER 已經推翻 03，成長位置必須跟著改後的解釋走。
禁止只換名稱：「妳正在學習好好溝通」「更能覺察自己的情緒」。
好的成長位置會改寫她對自己能力的定義，例如：不是永遠沒有情緒，而是有情緒時還能不能留一點選擇。
...
在使用者勾選、回答或用自己的話確認前，禁止寫「妳已經看見了」「你已經承認」。
...
const GROW_WRITE_SYSTEM = `你只把已通過的成長核心寫成可勾選的覺察方向。
語意必須等價。不要加新心理、新模式、新作業。
語氣溫柔、具體、穩定。讓她自己選擇，不命令、不說教。
不要把 AI 推測寫成她已確認的結論。

title：第一人稱，她可以認領。4～16 字。
text：短說明這個生活中做得到的方向。可以第二人稱。不要行動清單。
...
const LAUNDER = /你已經看見自己很害怕|你已經承認自己|你已經確認自己害怕|妳已經看見了|你已經看見了|妳已經承認|你已經承認/;
...
function gateCandidate(row, raw, answers, see, bag, ctx, meta) {
  ...
  if (looksLabelOnlyGrowth(text) || looksLabelOnlyGrowth(title)) failed.push("label-only");
  if (looksObviousLessonRelabel(blob, raw)) failed.push("obvious-lesson");
  if (
    !Boolean(row && row.bridge) &&
    thinkingCore.looksShallowParaphrase(text, highTrustBlob(raw, answers)) &&
    !thinkingCore.hasInterpretiveMove(text) &&
    !hasExplicitGrowthInRaw(raw, answers)
  ) {
    failed.push("parrot-raw");
  }
...
function emptyResult(raw, extra) {
  ...
  const bag = extra && extra.understand;
  meta.growTrace = {
    inputUnderstandStage: asText(bag && bag.stage),
    inputUnderstandStop: Boolean(bag && bag.stage === "stop"),
    candidateCountBefore: Number(meta._growCandidatesBefore) || 0,
    itemCountAfterGate: Number(meta._growAfterGate) || 0,
    itemCountAfterBoundary: 0,
    finalStatus: "empty",
    selectedIdsCount: 0,
    skipReason: asText(meta.skipReason) || null,
  };
  return {
...
function projectItems(items, raw, bag, extra) {
  ...
  const publicItems = items
    .slice(0, 3)
    .map((item, index) => {
      const text = publicBoundary.sanitizePublicText(item.text, {
        maxChars: limits.growText,
        fallback: "",
      });
      if (!text) return null;
      const title = publicBoundary.sanitizePublicText(item.title, {
        maxChars: limits.growTitle,
        fallback: "",
      });
      const out = {
  ...
  delete extraSafe.items;
  delete extraSafe.emptyCopy;
  delete extraSafe.status;
  delete extraSafe.selectedIds;
  const meta = extraSafe.meta && typeof extraSafe.meta === "object" ? extraSafe.meta : {};
  meta.growTrace = {
    inputUnderstandStage: asText(bag && bag.stage),
    inputUnderstandStop: Boolean(bag && bag.stage === "stop"),
    candidateCountBefore: Number(meta._growCandidatesBefore) || publicItems.length,
    itemCountAfterGate: Number(meta._growAfterGate) || publicItems.length,
    itemCountAfterBoundary: publicItems.length,
    finalStatus: publicItems.length ? "grow" : "empty",
    selectedIdsCount: 0,
    skipReason: asText(meta.skipReason) || null,
  };
  delete meta._growCandidatesBefore;
  delete meta._growAfterGate;
  return {
    variant: "awareness-v3",
    growVariant: GROW_VARIANT,
    status: publicItems.length ? "grow" : "empty",
    items: publicItems,
    selectedIds: [],
    sourceSig: awarenessV3.awarenessV3SourceSig({ ...raw, understand: bag }),
    observationCue: null,
    emptyCopy: publicItems.length
      ? null
      : {
          line1: publicBoundary.sanitizePublicText(EMPTY_COPY.line1, {
            maxChars: limits.growText,
            fallback: EMPTY_COPY.line1,
          }),
          line2: publicBoundary.sanitizePublicText(EMPTY_COPY.line2, {
            maxChars: limits.growText,
            fallback: EMPTY_COPY.line2,
          }),
        },
    ...extraSafe,
    meta,
...
【現行解釋｜可變假設，不是要證明的 03】
${core.interpretation || see || "（無）"}
狀態：${core.status || "empty"} ${core.revised ? "已被回答改過" : "尚未被回答改過"}
不要證明 03。若回答已推翻，跟著改後的解釋走。
...
若 USER ANSWER 指出「事後才後悔／當下沒空間／知道卻做不到」，這本身就是成長位置候選，不是空白收斂。
...
async function runGrowPipeline(options) {
  ...
  if (shouldSkipGrow(raw)) {
    return { ...emptyResult(raw, { understand: bag, meta: { ...meta, skipReason: "already-clear" } }), empty: false };
  }
  if (looksSilenceCascade(raw, bag, see, answers)) {
    return { ...emptyResult(raw, { understand: bag, meta: { ...meta, skipReason: "silence-cascade" } }), empty: false };
  }
  if (bag && bag.stage === "stop" && insightUnderstand.looksThinAmbiguous(raw) && !hasExplicitGrowthInRaw(raw, answers)) {
    return { ...emptyResult(raw, { understand: bag, meta: { ...meta, skipReason: "thin-ambiguous-stop" } }), empty: false };
  }
  ...
  meta._growCandidatesBefore = incoming.length;
  const kept = incoming
    .map((row) => gateCandidate(row, raw, answers, see, bag, ctx, meta))
    .filter(Boolean);
  meta._growAfterGate = kept.length;
  ...
  if ((reasonData && reasonData.stop && !kept.length) || !kept.length) {
    const bridge = upstreamGrowthBridge(raw, answers, see, bag);
    if (bridge) {
      const item = { ...bridge, id: "u1" };
      return {
        ...projectItems([item], raw, bag, { meta: { ...meta, bridge: true } }),
        empty: false,
        growStatus: "UPSTREAM_GROWTH_COMPLETE",
      };
    }
