...
const EXEC_DEEP_REFRESH_SYSTEM = `你是「進行式 ING」的行動整理者。使用者剛回答了 06 深度思考，請依新答案重新整理 3 個下一步。

保留今天 01～05 的核心，並吸收剛剛的回答。
已經被使用者選走的行動不要改寫、不要刪除、不要換句話說。
只為尚未選走的位置產出新的、有不同價值的行動。

仍然必須剛好 3 個（含已保留的）。
禁止 generic。禁止只是重複原話。禁止把已經做過的事再當下一步。
禁止重大人生決定指令。
...
function execDeepStep(body) {
  ...
  if (step === "refresh" || step === "close") return "refresh";
  return "ask";
}
...
function execRefreshUserPrompt(body) {
  ...
  const qa = execDeepAnsweredRounds(deep)
    .map((item, index) => `Q${index + 1}：${item.question}\n回答：${item.answer}`)
    .join("\n\n");
  return `請重新整理 3 個下一步。已選入執行力的行動必須原樣保留，只能改尚未選走的。

【必須保留】
${kept.length ? kept.map((item, index) => {
