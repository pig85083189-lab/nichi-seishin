...
2. 04 核心結論、盲點、改善方向，以及使用者自己回答的重要內容
不要把 04 改善方向原句當成 action。06 要轉成真正可執行的下一步。
3. 今日事件與使用者原話
4. 其他有必要的身心資訊
...
function formatExecutionStory(ctx, body) {
  ...
  const bodyNote = [ctx.bodyMindText || ctx.bodyNote, Array.isArray(ctx.bodyTags) ? ctx.bodyTags.join("、") : ""]
    .map((item) => String(item || "").trim())
    .filter(Boolean)
    .join("／");
  return `今日感謝：
...
心情：${mood}
身心原文：${bodyNote || "未寫"}
${ctx.bodyMindInsight ? `03 模型假設（非事實，不要重講）：${compactLine(ctx.bodyMindInsight, 80)}` : ""}`;
}
...
【04 ${v2 ? "核心結論" : "勾選／卡住的"}】${String(ctx.thinkCloseCore || ctx.thinkCloseAwareness || "").trim() || "未寫"}
【04 我沒看見的問題】${String(ctx.thinkCloseBlindSpot || "").trim() || "未寫"}
【04 改善方向（方向，不是 checklist。不要原句當 action）】${String(ctx.thinkCloseImprovement || ctx.thinkCloseDirection || "").trim() || "未寫"}
【04 使用者自己說出的】
...
04 核心：${compactLine(ctx.thinkCloseCore || ctx.thinkCloseAwareness, 120) || "未寫"}
04 盲點：${compactLine(ctx.thinkCloseBlindSpot, 120) || "未寫"}
04 方向：${compactLine(ctx.thinkCloseImprovement || ctx.thinkCloseDirection, 120) || "未寫"}
