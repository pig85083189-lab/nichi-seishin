 ...
 */
function toPublicUnderstand(bag) {
  const src = normalizeUnderstand(bag) || (bag && typeof bag === "object" ? bag : {});
  const stage = STAGES.includes(asText(src.stage)) ? asText(src.stage) : "";
  const limits = publicBoundary.LIMITS;
  const focus = publicBoundary.sanitizePublicText(src.focus, {
    maxChars: limits.understandFocus,
    fallback: "",
  });
  const whyWorthThinking = publicBoundary.sanitizePublicText(src.whyWorthThinking, {
    maxChars: limits.understandWhy,
    fallback: "",
  });
  const pastNote = publicBoundary.sanitizePublicText(src.pastNote, {
    maxChars: limits.understandPastNote,
    fallback: "",
  });
  const question = publicBoundary.sanitizePublicQuestion(src.question, {
    maxChars: limits.understandQuestion,
  });
  const question2 = publicBoundary.sanitizePublicQuestion(src.question2, {
    maxChars: limits.understandQuestion,
  });
  const convergence = publicBoundary.sanitizePublicText(src.convergence, {
...
function projectUnderstand(bag, raw, known) {
  const data = toPublicUnderstand(bag);
