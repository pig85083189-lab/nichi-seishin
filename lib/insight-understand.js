...
語氣溫柔、具體、穩定、有條理。先承接前面的覺察，再只問一個最值得確認、最自然的問題。
...
不要空泛文青或說教。不要假裝她已確認任何覺察。

focusLine：這次想陪她看的那一件事。一句到兩句。
why：為什麼值得想。短、落地；可先承接 03。
pastNote：只有真的用了過往才寫。要同時有相似與不一樣。可空。
question：通過的那一題，或 null。第一輪最多一題。
...
/**
 * PUBLIC client understand bag only.
 * Keeps past (needed for 05 grow round-trip). Strips possibilities/thinkingCore/pastDrop.
 * Sanitizes every user-facing AI string; leaves user answers untouched.
 * @param {object} bag
 * @param {{ boundaryDiag?: object }} [options]
 */
function toPublicUnderstand(bag, options) {
  const src = normalizeUnderstand(bag) || (bag && typeof bag === "object" ? bag : {});
  const stage = STAGES.includes(asText(src.stage)) ? asText(src.stage) : "";
  const limits = publicBoundary.LIMITS;
  const diag = options && options.boundaryDiag && typeof options.boundaryDiag === "object" ? options.boundaryDiag : null;

  const focusInsp = publicBoundary.inspectPublicText(src.focus, {
    maxChars: limits.understandFocus,
    fallback: "",
  });
  const whyInsp = publicBoundary.inspectPublicText(src.whyWorthThinking, {
    maxChars: limits.understandWhy,
    fallback: "",
  });
  const pastNoteInsp = publicBoundary.inspectPublicText(src.pastNote, {
    maxChars: limits.understandPastNote,
    fallback: "",
  });
  const questionInsp = publicBoundary.inspectPublicQuestion(src.question, {
    maxChars: limits.understandQuestion,
  });
  const question2Insp = publicBoundary.inspectPublicQuestion(src.question2, {
    maxChars: limits.understandQuestion,
  });
  const convergenceInsp = publicBoundary.inspectPublicText(src.convergence, {
    maxChars: limits.understandConvergence,
    fallback: "",
  });
  if (diag) {
    diag.focus = focusInsp.status;
    diag.why = whyInsp.status;
    diag.pastNote = pastNoteInsp.status;
    diag.question = questionInsp.status;
    diag.question2 = question2Insp.status;
    diag.convergence = convergenceInsp.status;
  }

  const past = src.past && typeof src.past === "object" ? src.past : null;
  return {
    variant: UNDERSTAND_VARIANT,
    stage: stage || asText(src.stage),
    focus: focusInsp.text,
    whyWorthThinking: whyInsp.text,
    pastNote: pastNoteInsp.text,
    past: past
      ? {
          used: Boolean(past.used || past.use),
          date: asText(past.date),
          similarity: publicBoundary.sanitizePublicText(past.similarity, {
            maxChars: limits.understandPastNote,
            fallback: "",
          }),
          difference: publicBoundary.sanitizePublicText(past.difference, {
            maxChars: limits.understandPastNote,
            fallback: "",
          }),
          change: publicBoundary.sanitizePublicText(past.change, {
            maxChars: limits.understandPastNote,
            fallback: "",
          }),
          connectionType: publicBoundary.sanitizePublicConnectionType(past.connectionType),
        }
      : null,
    question: questionInsp.text,
    answer: asText(src.answer),
    question2: question2Insp.text,
    answer2: asText(src.answer2),
    convergence: convergenceInsp.text,
    revised: Boolean(src.revised),
  };
}

function fieldPresence(bag) {
  const data = bag && typeof bag === "object" ? bag : {};
  const focus = asText(data.focus || data.focusLine);
  const why = asText(data.whyWorthThinking || data.why);
  const question = asText(data.question);
  return {
    focusPresent: Boolean(focus),
    focusLength: publicBoundary.compactChars(focus),
    whyPresent: Boolean(why),
    whyLength: publicBoundary.compactChars(why),
    questionPresent: Boolean(question),
    questionLength: publicBoundary.compactChars(question),
  };
}

function silenceReasonFromBoundary(focusStatus) {
  if (focusStatus === "reject_marker") return "boundary_reject_marker";
  if (focusStatus === "reject_length") return "boundary_reject_length";
  if (focusStatus === "empty") return "boundary_empty_focus";
  return "boundary_focus_lost";
}

function finishUnderstandTrace(meta, partial) {
  if (!meta || typeof meta !== "object") return;
  meta.understandTrace = {
    reasonerStatus: partial.reasonerStatus || "unknown",
    candidateCount: Number(partial.candidateCount) || 0,
    usableCandidateCount: Number(partial.usableCandidateCount) || 0,
    selectorStatus: partial.selectorStatus || "unknown",
    coreSelected: Boolean(partial.coreSelected),
    writerAttempted: Boolean(partial.writerAttempted),
    writerStatus: partial.writerStatus || "skipped",
    preBoundary: partial.preBoundary || fieldPresence({}),
    boundary: partial.boundary || {},
    postBoundary: partial.postBoundary || fieldPresence({}),
    projectionStatus: partial.projectionStatus || "unknown",
    finalStage: asText(partial.finalStage),
    finalStatus: asText(partial.finalStatus),
    silenceReason: partial.silenceReason || null,
  };
}

function silenceResult(raw, known, extra) {
  const meta = extra && extra.meta && typeof extra.meta === "object" ? extra.meta : {};
  const stopReason = asText(meta.stopReason);
  const copy = stopReason === "user-requested-stop" ? USER_STOP_COPY : STOP_COPY;
  const boundaryDiag = {};
  const understand = toPublicUnderstand(
    {
      variant: UNDERSTAND_VARIANT,
      stage: "stop",
      focus: "",
  ...
  finishUnderstandTrace(meta, {
    reasonerStatus: meta._traceReasonerStatus || (stopReason === "user-requested-stop" ? "skipped_user_stop" : stopReason === "already-clear" ? "skipped_already_clear" : stopReason === "model-stop" ? "stop" : meta.reasonError ? "error" : meta.writerFatal ? "ok" : "unknown"),
    candidateCount: meta._traceCandidateCount || 0,
    usableCandidateCount: meta._traceUsableCandidateCount || 0,
    selectorStatus: meta._traceSelectorStatus || (stopReason || meta.writerFatal ? meta._traceSelectorStatus || "failed" : "failed"),
    coreSelected: false,
    writerAttempted: Boolean(meta._traceWriterAttempted),
    writerStatus: meta.writerFatal ? "rejected" : meta._traceWriterStatus || "skipped",
    preBoundary: meta._tracePreBoundary || fieldPresence({}),
    boundary: boundaryDiag,
    postBoundary: fieldPresence(understand),
    projectionStatus: "silence",
    finalStage: "stop",
    finalStatus: "silence",
    silenceReason: stopReason || meta._traceSilenceReason || "selector_failed",
  });
  return {
...
function projectUnderstand(bag, raw, known, meta) {
  const preBoundary = fieldPresence(bag);
  const boundaryDiag = {};
  const data = toPublicUnderstand(bag, { boundaryDiag });
  ...
  let status = stage === "stop" ? "silence" : "understand";
  let projectionStatus = status === "silence" ? "silence" : "active";
  let silenceReason = status === "silence" ? asText(meta && meta.stopReason) || "stop" : null;
  // If public focus was wiped by the boundary, do not render an empty/leaky shell — use stop copy.
  if (status === "understand" && !data.focus) {
    status = "silence";
    projectionStatus = "silence_fallback";
    silenceReason = silenceReasonFromBoundary(boundaryDiag.focus);
    data.stage = "stop";
    data.question = "";
    data.question2 = "";
    data.whyWorthThinking = publicBoundary.sanitizePublicText(STOP_COPY.line1, {
      maxChars: publicBoundary.LIMITS.understandWhy,
      fallback: STOP_COPY.line1,
    });
    data.convergence = publicBoundary.sanitizePublicText(STOP_COPY.line2, {
      maxChars: publicBoundary.LIMITS.understandConvergence,
      fallback: STOP_COPY.line2,
    });
    data.pastNote = "";
  }
  if (meta && typeof meta === "object") {
    finishUnderstandTrace(meta, {
      reasonerStatus: meta._traceReasonerStatus || "ok",
      candidateCount: meta._traceCandidateCount || 0,
      usableCandidateCount: meta._traceUsableCandidateCount || 0,
      selectorStatus: meta._traceSelectorStatus || "selected",
      coreSelected: Boolean(preBoundary.focusPresent),
      writerAttempted: Boolean(meta._traceWriterAttempted),
      writerStatus: meta._traceWriterStatus || "ok",
      preBoundary,
      boundary: boundaryDiag,
      postBoundary: fieldPresence(data),
      projectionStatus,
...
async function runOpen(callAi, ctx, usedPast) {
  ...
  if (answerEngine.userAskedToStop(ctx)) {
    meta._traceReasonerStatus = "skipped_user_stop";
    meta._traceSelectorStatus = "skipped";
    meta._traceSilenceReason = "user-requested-stop";
  ...
  if ((looksNoUnknownLeft(raw, known)) && !seeGroundedInRaw(see, raw) && !unresolvedTension) {
    meta._traceReasonerStatus = "skipped_already_clear";
    meta._traceSelectorStatus = "skipped";
    meta._traceSilenceReason = "already-clear";
  ...
  try {
    ...
    meta._traceReasonerStatus = "error";
    meta._traceSelectorStatus = "skipped";
  ...
  meta._traceReasonerStatus = modelStop ? "stop" : "ok";
  meta._traceCandidateCount = Array.isArray(reasonData && reasonData.possibilities) ? reasonData.possibilities.length : 0;
  if (modelStop && looksNoUnknownLeft(raw, known) && !seeGroundedInRaw(see, raw) && !prematureStop) {
    meta._traceSelectorStatus = "skipped";
    meta._traceSilenceReason = asText(reasonData.stopReason) || "model-stop";
  ...
  if (!focus) {
    if (unresolvedTension) {
      ...
      if (forced) {
        ...
        selectorStatus = "forced";
      }
    }
  }
  meta._traceSelectorStatus = selectorStatus;
  meta._tracePreBoundary = fieldPresence({
    focus: focus && focus.statement,
    whyWorthThinking: focus && focus.whyWorthThinking,
    question: reasonData && reasonData.question,
  });
  if (!focus) {
    meta._traceSilenceReason = "selector_failed";
  ...
  meta._traceUsableCandidateCount = possibilities.length;
  ...
  meta._traceWriterAttempted = true;
  meta._traceWriterStatus = "ok";
  try {
    ...
    if (meta.writerRejected) meta._traceWriterStatus = "rejected_kept_core";
  } catch {
    meta._traceWriterStatus = "error_kept_core";
  }
  ...
  if (looksForcedPattern(`${written.focusLine} ${written.why}`) || insightDiscovery.looksOverreach(written.focusLine, written.why)) {
    if (!unresolvedTension) {
      meta._traceWriterStatus = "rejected";
      meta._traceSilenceReason = "writer_fatal";
      return { ...silenceResult(raw, known, { meta: { ...meta, writerFatal: true } }), empty: false };
    }
    meta.keptDespiteWriterFatal = true;
    meta._traceWriterStatus = "rejected_kept";
  }
  ...
  if (stage === "converged" && !written.question && looksThinAmbiguous(raw) && !unresolvedTension) {
    meta._traceSilenceReason = "thin-ambiguous";
  ...
  meta._tracePreBoundary = fieldPresence(bag);
  return {
    ...projectUnderstand(bag, raw, known, meta),
    empty: false,
    meta,
  };
}
...
async function runAnswer(callAi, ctx, priorInput) {
  ...
  if (!prior || !answer || compactChars(answer) < 4) {
    return { ...projectUnderstand(prior || { stage: "asked1", focus: "", whyWorthThinking: "" }, raw, known, meta), empty: true, meta };
  }
  ...
  try {
    ...
    meta._traceReasonerStatus = "error";
    meta._traceWriterAttempted = false;
    meta._traceWriterStatus = "skipped";
    return { ...projectUnderstand(fallback, raw, known, meta), empty: false, meta: { ...meta, reasonError: true } };
  }
  ...
  meta._traceReasonerStatus = "ok";
  meta._traceSelectorStatus = "selected";
  meta._traceWriterAttempted = true;
  meta._traceWriterStatus = "ok";
  meta._tracePreBoundary = fieldPresence(bag);
  return {
...
function scrubTraceScratch(meta) {
  if (!meta || typeof meta !== "object") return meta;
  Object.keys(meta).forEach((key) => {
    if (key.startsWith("_trace")) delete meta[key];
  });
  return meta;
}
...
async function runUnderstandPipeline(options) {
  ...
  const result =
    step === "answer" ? await runAnswer(callAi, ctx, opts.prior) : await runOpen(callAi, ctx, opts.usedPast || ctx.usedPast || []);
  if (result && result.meta) scrubTraceScratch(result.meta);
  return result;
}
