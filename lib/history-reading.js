(function (root, factory) {
  ...
  function piece(text, field, highlights, extra) {
    ...
    const detail = extra && extra.detail ? asText(extra.detail) : "";
    if (detail && detail !== value) next.detail = detail;
    return next;
  }
  ...
  function hasMemoryTurn(text) {
    return /不只是|同時|對我同樣|過程中|也要|而是/.test(String(text || "")) || hasContrastGain(text);
  }

  function hasMemoryQuoteGain(previousTexts, candidate) {
    const next = asText(candidate);
    if (!next) return false;
    const prev = (Array.isArray(previousTexts) ? previousTexts : [previousTexts]).map(asText).filter(Boolean);
    if (!prev.length) return true;
    ...
    const paraphrases = prev.filter((item) => looksLikeParaphrase(item, next));
    if (paraphrases.length) {
      if (hasMemoryTurn(next) && overlapScore(paraphrases.join(""), next) < 0.78) return true;
      return false;
    }
    const blob = prev.join("");
    if (sharedThemeAtoms(blob, next).length && compactKey(next).length <= 24 && !hasMemoryTurn(next)) {
      return false;
    }
    return true;
  }

  function pickMemoryQuote(candidates, usedTexts, usedFields) {
    const texts = Array.isArray(usedTexts) ? usedTexts.slice() : [];
    const fields = usedFields instanceof Set ? usedFields : new Set(usedFields || []);
    const list = Array.isArray(candidates) ? candidates : [];
    for (let i = 0; i < list.length; i += 1) {
      ...
      if (!item || !item.text) continue;
      if (item.field && fields.has(item.field)) continue;
      if (!hasMemoryQuoteGain(texts, item.text)) continue;
      return item;
    }
    return null;
  }
  ...
  function lightBodySignals(journal) {
    ...
    const mind = journal && journal.bodyMind && typeof journal.bodyMind === "object" ? journal.bodyMind : null;
    if (mind && asText(mind.text)) {
      lines.unshift({ field: "bodyMind.text", text: asText(mind.text) });
    }
  ...
  function collectActions(journal) {
    ...
    if (Array.isArray(selected) && selected.length) {
      return selected
        .map((item, index) =>
          piece(item && item.text, index === 0 ? "exec.smallestStep" : `exec.selected.${index}`, undefined, {
    ...
    const fromCards = cards
      .map((item, index) =>
        piece(
          ...
          `exec.item.${index}.title`,
          nestedHighlights(item && item.highlights, "title"),
          { detail: item && item.detail }
        )
      )
      .filter(Boolean);
  ...
  function stuckCandidates(journal, review, aware) {
    ...
    list.push(piece(guide.coreQuote, "think.coreQuote", fieldHighlights(marks, "coreQuote")));
    list.push(piece(close.coreConclusion, "think.awareness", fieldHighlights(marks, "awareness")));
  ...
  function seenCandidates(journal, aware) {
    ...
    const insightMarks = insight.highlights && typeof insight.highlights === "object" ? insight.highlights : {};
    const list = [];
    const v2 = String(guide.variant || "") === "think-v2";
    if (v2) {
      list.push(piece(aware.seen, "awareness.seen", fieldHighlights(aware.highlights, "seen")));
    ...
    const awareV3 =
      merge && typeof merge.selectedAwarenessV3Texts === "function"
        ? merge.selectedAwarenessV3Texts(journal && journal.awarenessV3)
        : [];
    (Array.isArray(awareV3) ? awareV3 : []).forEach((text, index) => {
      list.push(piece(text, `awareness.item.${index}`));
    });
    return list.filter(Boolean);
  }
  ...
  function hasDeepProcess(journal, review) {
    const { guide } = guideOf(journal);
    if (asText(guide && guide.coreQuote)) return true;
    if (Array.isArray(guide && guide.questions) && guide.questions.some((item) => asText(item && (item.text || item.question || item)))) return true;
  ...
  function hasAwareProcess(journal) {
    ...
    if (merge && typeof merge.hasMeaningfulAwarenessV3 === "function" && merge.hasMeaningfulAwarenessV3(journal && journal.awarenessV3)) return true;
    return false;
  }
  ...
  function buildHistoryReading(review) {
    ...
    const quote = pickMemoryQuote(quoteCandidates(journal, aware), [event, ...usedTexts], usedFields);
    if (quote) {
      usedFields.add(quote.field);
      usedTexts.push(quote.text);
    }

    const actions = [];
    collectActions(journal).forEach((item) => {
      if (!item || usedFields.has(item.field)) return;
      const fromChoices = String(item.field || "").startsWith("exec.smallestStep") || String(item.field || "").startsWith("exec.selected.");
      if (!fromChoices && actions.some((prev) => !hasInformationGain([prev.text], item.text))) return;
      usedFields.add(item.field);
      usedTexts.push(item.text);
      actions.push(item);
    });
