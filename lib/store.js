...
async function loadUserData(userId) {
  const id = assertUserId(userId);
  const fromSb = await loadSupabaseUserData(id);
  const [reviews, tasks, sfm, reports, insights, manifests] = await Promise.all([
  ...
  return {
    ...
    manifests: fromSb.manifests && fromSb.manifests.length ? fromSb.manifests : kvBundle.manifests,
  };
}
...
async function saveUserData(userId, bundle) {
  ...
  const manifests = Array.isArray(bundle.manifests)
    ? bundle.manifests.map((item) => ({ ...item, userId: id }))
    : readManifests(bundle, bundle.reports);
  await Promise.all([
    kvSet(userKey(id, "reviews"), reviews),
    kvSet(userKey(id, "tasks"), tasks),
    kvSet(userKey(id, "sfm"), sfm),
    kvSet(userKey(id, "insights"), insights),
    kvSet(userKey(id, "manifests"), manifests),
    kvSet(userKey(id, "reports"), reports),
  ]);
  await saveSupabaseUserData(id, { reviews, tasks, sfm, insights, manifests, reports }, { email: bundle.email || "" });
  return { userId: id, reviews, tasks, sfm, insights, manifests, reports };
}
