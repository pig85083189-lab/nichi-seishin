...
const { pickReview, mergeReviewMaps } = require("./review-merge");
...
async function mergeReviews(userId, incoming) {
  ...
  Object.entries(incoming || {}).forEach(([iso, review]) => {
    ...
    next[iso] = pickReview(current[iso], { ...review, date: iso, userId: assertUserId(userId) });
  });
...
async function loadUserData(userId, extra = {}) {
  ...
  const merged = stampBundleUser(id, mergeUserBundles(kvBundle, fromSb));
  return { ...merged, updatedAt: fromSb.updatedAt || "" };
}
