...
stuck：今天真正看見的核心。1～2 句。可短。不要重述「好好溝通」。
seen：他說出了什麼。回扣原話。不要發明領悟。
unknown：只有他自己留下未知時才寫。不要創造。沒有就 ""。
direction：可以怎麼變得更好。1～3 句方向／方法／需驗證的關鍵。不是 checklist、不是明天待辦。
若已經很清楚、沒有更深：direction 可寫「這件事目前沒有需要再被解釋得更深。」
title：8-16 字。具體。
...
同時回答：1) 現在真正看見的核心 2) 可以怎麼變得更好（方向，不是 06 checklist）。
...
function normalizeThinkV2Close(raw, body) {
  ...
  return {
    ...
    selfSeen: seen,
    takeaway: unknown,
    direction: stripInventedDepth(data.direction || data.improve || ""),
    actions: [],
  };
}
