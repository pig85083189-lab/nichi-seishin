...
禁止把使用者已經知道的答案，重新包裝成問題。
已說「要溝通／表達／被理解」：不要再問怎麼溝通、希望對方理解什麼。
問還沒看見、且答案會改變理解的那一件。
03 模型假設不是事實；使用者否定就立刻放掉。
...
const THINK_V2_CLOSE_SYSTEM = `你是一面鏡子。只把剛才被說出來的東西收成三個不同資訊的區塊。

CLOSE DEPTH <= EVIDENCE DEPTH
Close 的深度不能超過使用者實際說出的深度。不要比他更聰明。
有些事情看清楚之後，沒有更深的東西。不要補人格、模式、二次解讀。

不要提問、不要 06 checklist、不要雞湯、不要心理診斷。
不要「你通常／你一直／長期以來」。
禁止：「問題解決模式」「二次解讀」「你習慣進入……」。
禁止強行找童年創傷、不安全感、內在小孩。不要每件事都心理化。

證據優先：
1) 使用者 04 回答
2) 使用者 03 bodyMind.text 原文
3) 今日事件原文
4) 03 insight 只能當 hypothesis；被否定就不能寫回去

coreConclusion（核心結論）：經過剛才回答，今天這件事真正的核心是什麼。1～2 句。
不是重述事件、不是重述情緒、不是把原話換句話說。
evidence 不足就只說目前能確認的核心，不要為了看起來深而補故事。
使用者自己留下的未知（還沒想清楚／不知道有沒有選擇）必須寫進核心能確認的範圍，不要清空、不要發明答案。

blindSpot（我沒看見的問題）：使用者可能還沒注意到、但會影響她理解這件事的一個盲點。
任務是 WHAT AM I NOT SEEING YET。要讓人覺得「對，我剛剛沒想到」。
不是再問一個問題、不是重述核心、不是把 03 insight 換句話說。
沒有足夠資訊就寫：「目前沒有明顯需要再往深處解讀的地方。」
或：「目前更需要確認事實，而不是再替這件事增加新的解釋。」
疲憊可以停在「今天只是累」。幸福不要硬找問題。

improvementDirection（怎麼做可以更好）：知道核心和盲點之後，事情可以往哪個方向變好。1～3 句方向。
不是 checklist、不是「晚上 8 點傳訊息／寫下三件事／跟媽媽說某一句話」。那些屬於 06。

三段必須各自提供不同 information value。禁止同一句話換三種寫法。
生成前做 semantic redundancy check。例如禁止：
核心「你希望媽媽理解你」／盲點「你可能沒發現自己希望被理解」／改善「可以讓媽媽更理解你」。
...
function looksKnownAnswerRestate(question, ctx, rounds) {
  const q = String(question || "");
  const blob = [
    ctx && ctx.event,
    ctx && ctx.bodyNote,
    ctx && ctx.bodyMindText,
    (Array.isArray(rounds) ? rounds : []).map((item) => `${item && item.question} ${item && item.answer}`).join("\n"),
  ]
    .map((item) => String(item || ""))
    .join("\n");
  if (/溝通|好好說話|表達自己|跟媽媽說/.test(blob) && /你想怎麼.{0,10}(溝通|表達|說話)|怎麼跟.{0,10}(溝通|說)|可以怎麼.{0,8}溝通/.test(q)) {
    return true;
  }
  if (/理解我|希望她懂|希望被理解|理解你的感受/.test(blob) && /你希望.{0,10}理解|希望對方理解什麼|想讓.{0,8}理解什麼/.test(q)) {
    return true;
  }
  return false;
}
...
function formatBody(ctx) {
  ...
  const raw = String(ctx.bodyMindText || "").trim() || String(ctx.bodyNote || "").trim();
  const hypothesis = String(ctx.bodyMindInsight || "").trim();
  const lines = [];
  if (raw) lines.push(`03 身心覺察原文（使用者自己寫的，權重最高）：${raw}`);
  if (hypothesis) lines.push(`03 模型假設（hypothesis，不是使用者確認的事實。若後面否定立刻放掉）：${hypothesis}`);
  ...
  if (!check) {
    ...
    if (!lines.length && !tags) return "";
    if (tags && !raw) lines.push(`身體狀態：${tags}`);
    return lines.join("\n");
  }
  ...
  [groupLine("今日心情檢核", check.mood), groupLine("今日身體檢核", check.body), sleepBits.length
...
function scoreInformationGain(options) {
  ...
  if (looksKnownAnswerRestate(question, ctx, rounds)) {
    return { score: 0, reason: "known-restate", gainKind: GAIN_NONE };
  }
...
${formatRounds(rounds)}`;
  ...
  if (String(body && body.step || ctx.step || "ask") === "close") {
    return `只收斂剛才被說出來的東西。深度不能超過證據。
固定輸出三個資訊不同的區塊：coreConclusion / blindSpot / improvementDirection。
不要提問。不要 06 checklist。不要重述「好好溝通」。不要寫模式／人格／你通常。
03 假設不是事實。被否定的假設不能寫回去。
使用者自己留下的未知必須寫進 coreConclusion 目前能確認的範圍；沒有就不要發明。
...
const BLIND_SPOT_FALLBACK = "目前沒有明顯需要再往深處解讀的地方。";
const BLIND_SPOT_FACT_FALLBACK = "目前更需要確認事實，而不是再替這件事增加新的解釋。";
...
function looksChecklist(text) {
  const value = String(text || "");
  return /晚上\s*\d|寫下三件|傳訊息|跟媽媽說[「『]|步驟[一二三123]|^\s*[1１][\.．、]|\n\s*[2２][\.．、]/.test(value);
}

function looksOverPsych(text) {
  return /內在小孩|童年創傷|不安全感|依附風格|習慣把別人的需求|心理診斷|人格模式/.test(String(text || ""));
}

function resolveThinkClose(guide) {
  const data = guide && typeof guide === "object" ? guide : {};
  const stored = data.close && typeof data.close === "object" ? data.close : {};
  const coreConclusion = String(
    stored.coreConclusion || data.coreConclusion || data.stuck || data.awareness || data.summary || ""
  ).trim();
  const blindSpot = String(stored.blindSpot || data.blindSpot || "").trim();
  const improvementDirection = String(
    stored.improvementDirection || data.improvementDirection || data.direction || ""
  ).trim();
  return { coreConclusion, blindSpot, improvementDirection };
}

function evaluateThinkV2Close(close, options) {
  const resolved = resolveThinkClose(close);
  const core = resolved.coreConclusion;
  const blind = resolved.blindSpot;
  const improve = resolved.improvementDirection;
  const blob = `${core}${blind}${improve}`;
  const issues = [];
  if (!core) issues.push("missing-core");
  if (!blind) issues.push("missing-blind");
  if (!improve) issues.push("missing-improve");
  if (looksSemanticDuplicate(core, blind)) issues.push("blind-restates-core");
  if (looksSemanticDuplicate(core, improve)) issues.push("improve-restates-core");
  if (looksSemanticDuplicate(blind, improve)) issues.push("improve-restates-blind");
  if (looksChecklist(improve)) issues.push("improve-is-checklist");
  if (looksOverPsych(blob) && !(options && options.allowPsych)) issues.push("overpsych");
  const rejected = (options && options.rejected) || [];
  rejected.forEach((item) => {
    const note = String(item && (item.note || item.topic || item) || "").trim();
    if (note && blob.includes(note.slice(0, 8))) issues.push("rejected-revived");
  });
  if (options && options.forbid && options.forbid.test(blob)) issues.push("unsupported");
  return { ok: !issues.length, issues, ...resolved };
}
...
function normalizeThinkV2Close(raw, body) {
  ...
  const ctx = body && body.context && typeof body.context === "object" ? body.context : {};
  const stored = data.close && typeof data.close === "object" ? data.close : {};
  let coreConclusion = stripInventedDepth(
    stored.coreConclusion || data.coreConclusion || data.stuck || data.awareness || ""
  );
  let blindSpot = stripInventedDepth(stored.blindSpot || data.blindSpot || "");
  let improvementDirection = stripInventedDepth(
    stored.improvementDirection || data.improvementDirection || data.direction || data.improve || ""
  );
  ...
  if (unknown && coreConclusion && !closeTextKey(coreConclusion).includes(closeTextKey(unknown).slice(0, 6))) {
    coreConclusion = `${coreConclusion}目前能確認的是到這裡；${unknown}。`.replace(/。+/g, "。");
  } else if (unknown && !coreConclusion) {
    coreConclusion = `目前能確認的核心還有限：${unknown}`;
  }
  if (!blindSpot) blindSpot = BLIND_SPOT_FALLBACK;
  if (looksSemanticDuplicate(coreConclusion, blindSpot)) blindSpot = BLIND_SPOT_FALLBACK;
  if (looksChecklist(improvementDirection)) improvementDirection = "";
  ...
  return {
    ...
    stuck: coreConclusion,
    seen,
    unknown,
    awareness: coreConclusion,
    selfSeen: seen,
    takeaway: unknown,
...
module.exports = {
  ...
  looksKnownAnswerRestate,
  looksSemanticDuplicate,
  looksChecklist,
  looksOverPsych,
  resolveThinkClose,
  evaluateThinkV2Close,
  shouldSkipThinkV2Ask,
  heuristicEarlyClose,
  thirdAskPreconditions,
  BLIND_SPOT_FALLBACK,
  BLIND_SPOT_FACT_FALLBACK,
};
