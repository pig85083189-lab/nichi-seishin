const { getAdminClient, isInternalUser, listInternalUserIds } = require("./supabase");
...
const UNIQUE_ONCE = new Set(["auth_signup_completed", "trial_started", "trial_expired", "subscription_started", "plus_interest_clicked"]);
...
function eventCategory(name) {
  ...
  if (name.includes("trial") || name.includes("subscription_started") || name.startsWith("plus_")) return "billing";
  return "product";
}
...
async function insertAnalyticsEvent(input) {
  ...
  if (await isInternalUser(userId)) return { ok: true, skipped: true, internal: true };
  const admin = adminClient();
...
function plusConversionFrom(events) {
  const layers = [
    { key: "plus_offer_viewed", label: "看到 PLUS" },
    { key: "plus_plan_viewed", label: "查看 PLUS" },
    { key: "plus_interest_clicked", label: "我想升級" },
  ];
  const counted = layers.map((layer) => {
    const matched = (events || []).filter((event) => event.event_name === layer.key);
    return {
      ...layer,
      users: new Set(matched.map((event) => event.user_id)).size,
      events: matched.length,
    };
  });
  return counted.map((layer, index) => {
    const prev = index === 0 ? layer.users : counted[index - 1].users;
    return {
      ...layer,
      fromPrev: prev ? layer.users / prev : 0,
    };
  });
}
...
async function loadAnalyticsBundle() {
  const [events, profiles, subscriptions, cohorts, members, internalUserIds] = await Promise.all([
  ...
  return { events, profiles, subscriptions, cohorts, members, internalUserIds };
}

function excludeInternal(rows, internalIds, idKey = "id") {
  if (!internalIds || !internalIds.size) return rows || [];
  return (rows || []).filter((row) => !internalIds.has(String((row && row[idKey]) || "")));
}

function buildDashboard(bundle, options = {}) {
  const cohortSlug = String(options.cohort || "all").trim();
  const internalIds = new Set((bundle.internalUserIds || []).map((id) => String(id || "").trim()).filter(Boolean));
  const events = excludeInternal(bundle.events, internalIds, "user_id");
  const profiles = excludeInternal(bundle.profiles, internalIds, "id");
  const subscriptions = excludeInternal(bundle.subscriptions, internalIds, "user_id");
  const memberIds = cohortSlug && cohortSlug !== "all"
    ? bundle.members.filter((row) => {
        ...
        return cohort && row.cohort_id === cohort.id && !internalIds.has(String(row.user_id || ""));
      }).map((row) => row.user_id)
    : null;
  const users = buildUserMaps(events, profiles, subscriptions);
  const scopedUsers = filterUsers(users, memberIds);
  const scopedEvents = memberIds
    ? events.filter((event) => memberIds.includes(event.user_id))
    : events;
  ...
  return {
    ...
    plusConversion: plusConversionFrom(scopedEvents),
